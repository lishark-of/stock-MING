...
class CandidateRadarOrdinaryEntryTests(unittest.TestCase):
    ...
    def test_search_quant_projection_keeps_task_boundary_visible(self):
        ...
        self.assertIn("P2/P3 回放索引", self.page)
        self.assertIn("直接读取 packet 顶层 counts / policy", self.page)
        self.assertIn("确认回执、任务回放、provider API 和 P3 结果速读都只做本地回放", self.page)
        self.assertIn('索引: "确认回执"', self.page)
        self.assertIn('索引: "任务回放"', self.page)
        self.assertIn('索引: "Provider API"', self.page)
        self.assertIn('索引: "P3 结果速读"', self.page)
        ...
        self.assertIn("provider API 行只读回放 ledger 状态；React render 不调用 provider/model", self.page)
        ...
        self.assertLess(ordinary_result_quick_index, deepseek_governance_index)
