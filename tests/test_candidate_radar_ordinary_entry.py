...
class CandidateRadarOrdinaryEntryTests(unittest.TestCase):
    ...
    def test_search_quant_projection_keeps_task_boundary_visible(self):
        ...
        self.assertIn("点击确认后先看这里：任务是否接收、task id 是否可见、cache / ledger / packet 是否可回放；这张速读表不创建第二个任务。", self.page)
        ...
        self.assertIn("先看可读结论、回放来源和待补证据", self.page)
        ...
        self.assertIn("href #factor 只切换到本地量化推演入口；不发 POST task、不调 Tushare/DeepSeek", self.page)
        self.assertIn("href #next 只切换到本地次日图谱入口；不生成交易动作、不覆盖 strategy action", self.page)
        self.assertIn("Radar candidate 不是交易指令；真实交易路径继续隔离", self.page)
        ...
        self.assertLess(ordinary_result_quick_index, replay_destinations_index)
    ...
    def test_ordinary_quant_projection_submit_does_not_auto_chain_provider_model(self):
        ...
        self.assertIn("TaskStatusPanel", post_confirm_slice)
