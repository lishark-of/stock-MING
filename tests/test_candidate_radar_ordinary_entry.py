...
class CandidateRadarOrdinaryEntryTests(unittest.TestCase):
    ...
    def test_candidate_radar_has_ordinary_user_summary_before_audit_details(self):
        ...
        self.assertIn('aria-label="candidate radar compact operator research assist details"', operator_slice)
        self.assertIn("<summary>研究辅助 / 操作台详情</summary>", operator_slice)
        self.assertIn("默认先保留输入框、最近结果和主入口", operator_slice)
        operator_details_start = operator_slice.index('aria-label="candidate radar compact operator research assist details"')
        operator_details_end = operator_slice.index("退旧雷达前还缺什么：", operator_details_start)
        operator_details_slice = operator_slice[operator_details_start:operator_details_end]
        self.assertLess(compact_actions_start, operator_details_start)
        self.assertIn("candidateRadarOperatorPostConfirmOneGlanceItems", self.page)
        self.assertIn('aria-label="candidate radar operator post confirm one glance result"', operator_details_slice)
        self.assertIn("确认后马上看这里", operator_details_slice)
        self.assertIn('aria-label="candidate radar operator post confirm one glance sentence"', operator_details_slice)
        self.assertIn("MetricGrid items={ordinaryUserMetricItems(candidateRadarOperatorPostConfirmOneGlanceItems)}", operator_details_slice)
        ...
        self.assertIn('aria-label="candidate radar post confirm data capability card"', operator_details_slice)
        self.assertIn("确认后数据能力", operator_details_slice)
        self.assertIn('aria-label="candidate radar post confirm data capability sentence"', operator_details_slice)
        self.assertIn("MetricGrid items={ordinaryUserMetricItems(candidateRadarPostConfirmDataCapabilityItems)}", operator_details_slice)
        self.assertIn('aria-label="candidate radar operator post confirm one glance actions"', operator_details_slice)
        self.assertIn("操作台确认后结果条只读本地确认记录、本地缓存、数据记录和结果包", operator_details_slice)
        ...
        self.assertGreater(operator_post_confirm_start, operator_details_start)
        ...
        self.assertIn('href="#storage"', recent_result_slice)
        self.assertIn('aria-label="open storage current last good from recent research result card"', recent_result_slice)
        ...
        self.assertIn('href="#factor/factor-score"', action_slice)
        self.assertIn('href="#next/next-session-chart"', action_slice)
        ...
        self.assertIn('id="candidate-pool-details"', self.page)
        self.assertEqual(self.page.count('id="candidate-pool"'), 1)
        self.assertEqual(self.page.count('id="candidate-pool-details"'), 1)
        self.assertIn('title="候选池速读"', self.page)
        self.assertIn('aria-label="candidate pool compact anchor conclusion"', self.page)
        self.assertIn('aria-label="open candidate pool full details from compact anchor"', self.page)
        self.assertIn('href="#candidates/candidate-pool-details"', self.page)
        self.assertIn('href="#candidates/candidate-radar-search-quant-projection"', self.page)
        self.assertLess(self.page.index('title="下一票雷达操作台"'), self.page.index('id="candidate-pool"'))
        self.assertLess(self.page.index('id="candidate-pool"'), self.page.index('title="普通用户雷达摘要"'))
        self.assertLess(self.page.index('id="candidate-pool"'), self.page.index('id="candidate-pool-details"'))
    ...
    def test_candidate_pool_current_result_card_is_local_navigation_only(self):
        ...
        self.assertIn('aria-label="open storage current last good from current result card"', pool)
        ...
        self.assertIn('href="#storage"', pool)
    ...
    def test_p1_symbol_input_is_shared_and_silent_until_confirm(self):
        ...
        self.assertEqual(self.page.count("updateSearchSymbolInput(event.target.value);"), 5)
        ...
        self.assertEqual(self.page.count("renderQuantProjectionPrimaryAction("), 5)
        ...
        self.assertIn('id="candidate-focus-symbol"', self.page)
        ...
        self.assertIn('href="#storage"', primary_action_slice)
        self.assertIn('aria-label="open storage current last good from candidate recent result primary action"', primary_action_slice)
