...
class CandidateRadarOrdinaryEntryTests(unittest.TestCase):
    ...
    def test_candidate_radar_has_ordinary_user_summary_before_audit_details(self):
        ...
        self.assertIn("P4 将 P5 治理状态下沉到开发审计区", self.page)
        ...
        for name, config in pages.items():
            with self.subTest(name=name):
                ...
                if name == "daily":
                    ...
                    self.assertIn("确认后 Tushare 数据卡", text)
                    self.assertIn('aria-label="daily command p1 tushare data card summary"', text)
                    self.assertIn("MetricGrid items={dailyCommandTushareDataCardItems}", text)
                    self.assertIn('label: "Tushare 数据卡"', text)
    ...
    def test_search_quant_projection_keeps_task_boundary_visible(self):
        ...
        self.assertIn("!searchSymbolTouched && !searchSymbol.trim() && normalizeAshareSymbolInput(cachedQuantProjectionSymbol).valid", self.page)
        self.assertIn("setSearchSymbol(cachedQuantProjectionSymbol);", self.page)
        ...
        self.assertIn("只读回放 task receipt / cache / ledger / packet；不创建第二个 task、不交易、不改交易策略", self.page)
        ...
        self.assertIn("只有确认按钮创建本地 POST task；DeepSeek skipped；不交易", self.page)
        ...
        self.assertIn("高级：P5 DeepSeek 单独补证", self.page)
        self.assertIn("P5 只作为高级单独补证", self.page)
        self.assertIn("不作为普通用户当前下一步，不阻塞 P1/P2/P3 本地回放", self.page)
    ...
    def test_p1_symbol_input_is_shared_and_silent_until_confirm(self):
        ...
        self.assertEqual(self.page.count("setSearchSymbol(event.target.value);"), 4)
        self.assertEqual(self.page.count("setSearchSymbolTouched(true);"), 4)
