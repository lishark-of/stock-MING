...
class TaskCatalogOrdinaryEntryTests(unittest.TestCase):
    def test_task_monitor_shows_ordinary_progress_before_engineering_catalog(self):
        ...
        self.assertIn("taskCatalogIndexSourceTushareReplayed", source)
        self.assertIn("taskCatalogIndexTushareReplayLabel", source)
        self.assertIn("taskCatalogOrdinaryReadbackLabel", source)
        self.assertIn("taskCatalogLatestIsCandidateReplay", source)
        self.assertIn("taskCatalogLatestConfirmedSymbol", source)
        self.assertIn("taskIndex?.latest_confirmed_symbol", source)
        self.assertIn("taskCatalogLatestConfirmedSymbolLabel", source)
        self.assertIn("当前标的：${taskCatalogLatestConfirmedSymbol}", source)
        self.assertIn("taskCatalogLatestConfirmedTaskLabel", source)
        ...
        self.assertIn('label: "GET 回放外联"', source)
        self.assertIn('label: "源任务 Tushare"', source)
        self.assertIn('label: "目录 GET 外联"', source)
        self.assertIn("Tushare-first ledger 已从 CandidateRadar cache 回放", source)
        self.assertIn("源任务 Tushare 已从 call_ledger 回放", source)
        ...
        self.assertNotIn('label: "任务外联"', source)
        self.assertNotIn('label: "已触发外部调用"', source)
