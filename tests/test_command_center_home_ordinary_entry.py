...
class CommandCenterHomeOrdinaryEntryTests(unittest.TestCase):
    ...
    def test_usable_path_progress_card_is_front_loaded_and_read_only(self):
        ...
        self.assertIn("document.getElementById(anchor)?.scrollIntoView({ block: \"start\" })", app_source)
        self.assertIn("window.requestAnimationFrame(scrollToAnchor)", app_source)
        self.assertIn("window.setTimeout(scrollToAnchor, 120)", app_source)
