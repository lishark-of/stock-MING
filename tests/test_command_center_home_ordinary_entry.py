...
class CommandCenterHomeOrdinaryEntryTests(unittest.TestCase):
    ...
    def test_daily_command_center_summary_shows_user_decision_fields_first(self):
        ...
        self.assertIn("dailyCommandConfirmedChainResolution", source)
        self.assertIn("selectMatchingHomeConfirmedChain", source)
        ...
        self.assertIn("MetricGrid items={ordinaryHomeMetricItems(ordinaryHomeStatusItems)}", first_card_slice)
    ...
    def test_ordinary_home_app_visible_now_summary_is_first_screen_read_only(self):
        ...
        self.assertIn("结果版本 ${dailyCommandCurrentResultVersion || \"等待 result_version\"}", source_before_audit)
        self.assertIn("ordinaryHomeCanonicalResultInline", source_before_audit)
        self.assertIn("candidateQuantResultVersionSummary.canonical_model_ledger_id", source_before_audit)
        ...
        self.assertIn("MetricGrid items={ordinaryHomeMetricItems(ordinaryHomeAppVisibleNowItems)}", visible_slice)
        ...
        self.assertIn('href={dailyCommandCandidateConfirmHref}', visible_slice)
        self.assertIn('href="#marginEtf"', visible_slice)
        self.assertIn('aria-label="open candidate radar from home visible now summary"', visible_slice)
        self.assertIn('aria-label="open margin etf from home visible now summary"', visible_slice)
        ...
        self.assertIn('aria-label="open storage current result from home visible now summary"', visible_slice)
    ...
    def test_usable_path_progress_card_is_front_loaded_and_read_only(self):
        ...
        self.assertIn("window.setTimeout(scrollToAnchor, delayMs)", app_source)
        self.assertIn("retryTimers.forEach((timer) => window.clearTimeout(timer));", app_source)
        self.assertIn("const [hashScrollVersion, setHashScrollVersion] = useState(0);", app_source)
        self.assertIn("setHashScrollVersion((value) => value + 1);", app_source)
        self.assertIn("[route, localFastapiRefreshNonce, hashScrollVersion]", app_source)
        self.assertIn('data-local-fastapi-refresh-boundary="local_health_only_no_route_remount"', app_source)
        self.assertIn("<ActiveRoute key={route} />", app_source)
        self.assertNotIn("key={`${route}:${localFastapiRefreshNonce}`}", app_source)
    ...
    def test_home_routes_data_gaps_to_data_capability_without_new_task(self):
        ...
        self.assertIn("MetricGrid items={ordinaryHomeMetricItems(ordinaryHomeAppVisibleNowItems)}", visible_card)
        ...
        self.assertIn("真实数据补证仍需单独授权；首页不会自动补调，也不会把缺口当作可交易结论", source)
    ...
    def test_ordinary_home_first_card_shows_result_routes_without_task_trigger(self):
        ...
        self.assertIn("ordinaryHomeStorageCurrentDataDate", source)
        self.assertIn("ordinaryHomeStorageCurrentFreshness", source)
        self.assertIn("ordinaryHomeStorageCurrentFreshnessLabel", source)
        self.assertIn("ordinaryHomeStoragePromotionInline", source)
        self.assertIn("ordinaryHomeResultVersionGuardInline", source)
        ...
        self.assertIn("MetricGrid items={ordinaryHomeMetricItems(ordinaryHomePlainConclusionItems)}", recent_result)
        self.assertIn('aria-label="ordinary home recent result local readback actions"', recent_result)
        self.assertIn('aria-label="open factor result from ordinary home recent result"', recent_result)
        self.assertIn('aria-label="open next result from ordinary home recent result"', recent_result)
        self.assertIn('aria-label="open storage current result from ordinary home recent result"', recent_result)
        self.assertIn('href="#storage"', recent_result)
        ...
        self.assertIn("MetricGrid items={ordinaryHomeMetricItems(ordinaryHomeRecentResultItems)}", recent_result)
        self.assertIn('label: "缺口/degraded"', source)
        self.assertIn('label: "Storage 提升任务"', source)
        ...
        storage_binding_start = source.index("const ordinaryHomeStorageBinding")
        storage_binding_end = source.index("const ordinaryHomeStorageCurrentSymbol", storage_binding_start)
        storage_binding = source[storage_binding_start:storage_binding_end]
        self.assertIn('dataDate: "data_date"', storage_binding)
        self.assertIn('freshness: "freshness_state"', storage_binding)
        self.assertIn('taskId: "source_result_task_id"', storage_binding)
        self.assertNotIn("ordinaryHomeStorageCurrentResult.", storage_binding)
        ...
        self.assertIn("确认后数据回放", tushare_data_card)
        ...
        self.assertIn("MetricGrid items={ordinaryHomeMetricItems(dailyCommandTushareDataCardItems)}", tushare_data_card)
        self.assertIn("candidateQuantResultVersionSummary", source)
        self.assertIn("candidateQuantCurrentResultLineage", source)
        self.assertIn("candidateQuantLastGoodResultLineage", source)
        self.assertIn("candidateQuantDegradedResultLineage", source)
        self.assertIn("dailyCommandCurrentResultLabel", source)
        self.assertIn("dailyCommandLatestResultLabel", source)
        self.assertIn("dailyCommandLastGoodResultLabel", source)
        self.assertIn("dailyCommandResultVersionGuardLabel", source)
        self.assertIn('label: "当前可用结果"', source)
        self.assertIn('label: "最新尝试"', source)
        self.assertIn('label: "last-good"', source)
        self.assertIn('label: "覆盖保护"', source)
        self.assertIn("首页 current 仍保留", source)
        self.assertIn("旧任务不会覆盖 current", source)
        self.assertIn("旧任务不能覆盖 current；只按 symbol + result_version 提升结果", source)
        ...
        self.assertIn("MetricGrid items={ordinaryHomeMetricItems(ordinaryHomeCandidateRadarItems)}", candidate_radar)
        ...
        self.assertIn("MetricGrid items={ordinaryHomeMetricItems(ordinaryHomeResultRouteItems)}", route)
    ...
    def test_ordinary_home_user_route_qa_quick_read_is_visible_and_read_only(self):
        ...
        self.assertIn("MetricGrid items={ordinaryHomeMetricItems(ordinaryHomeUserRouteQaItems)}", qa)
        for label in (
            'label: "路线覆盖"',
            'label: "视口覆盖"',
            'label: "输入静默"',
        ...
        self.assertIn("不是外部数据或模型验收、不是远端发布检查，也不是最终完成声明", qa)
    ...
    def test_ordinary_home_first_card_shows_post_confirm_status_without_second_task(self):
        ...
        self.assertIn("ordinaryHomeConfirmResultChainSentence", source)
        self.assertIn("ordinaryHomeConfirmResultChainItems", source)
        ...
        self.assertIn("确认后结果链路等待开始：先输入股票并点击确认，随后按进度、本地记录、结果入口顺序看。", source)
        self.assertIn("只读当前结果链；普通链接只切换本地页面；不调用外部服务、不交易、不改策略", source)
        ...
        self.assertIn("MetricGrid items={ordinaryHomeMetricItems(ordinaryHomePostConfirmItems)}", status)
        self.assertIn('aria-label="ordinary home confirm result chain"', status)
        self.assertIn("确认后结果链路", status)
        self.assertIn('aria-label="ordinary home confirm result chain summary"', status)
        self.assertIn("MetricGrid items={ordinaryHomeMetricItems(ordinaryHomeConfirmResultChainItems)}", status)
        self.assertIn('aria-label="ordinary home confirm result chain actions"', status)
        self.assertIn('aria-label="open progress from ordinary home confirm result chain"', status)
        self.assertIn('aria-label="open factor from ordinary home confirm result chain"', status)
        self.assertIn('aria-label="open next session from ordinary home confirm result chain"', status)
        self.assertIn("这里顺序只读当前确认、本地进度、本地记录和结果入口", status)
        ...
        self.assertIn("MetricGrid items={ordinaryHomeMetricItems(ordinaryHomePostConfirmReplayItems)}", status)
        ...
        self.assertLess(status.index('aria-label="ordinary home confirm result chain"'), status.index('aria-label="ordinary home post confirm replay state strip"'))
        self.assertLess(status_start, route_start)
        chain_start = status.index('aria-label="ordinary home confirm result chain"')
        chain_end = status.index('aria-label="ordinary home post confirm replay state strip"', chain_start)
        chain = status[chain_start:chain_end]
        self.assertNotIn("onClick=", chain)
        self.assertNotIn("fetch(", chain)
        self.assertNotIn("postCandidateRadar", chain)
        self.assertNotIn("TaskStatusPanel", chain)
    ...
    def test_home_p1_confirm_handoff_keeps_receipt_audit_folded(self):
        ...
        self.assertIn("taskIndexConfirmedChainSafe", source)
        self.assertIn("selectMatchingHomeConfirmedChain", source)
        self.assertIn("makeStrictHomeConfirmedChain", source)
        self.assertIn("latest_confirmed_symbol_readback_external_calls_triggered", source)
        self.assertIn("latest_confirmed_symbol_creates_task_from_readback", source)
        ...
        self.assertIn("当前标的自动填入只来自 CandidateRadar cache / GET /api/tasks", source)
        ...
        self.assertIn("dailyCommandFactorCandidateHandoffReady", source)
        self.assertIn("CandidateRadar cache / GET /api/tasks 最近确认 task", source)
    ...
    def test_daily_command_center_source_and_boundary_are_visible(self):
        ...
        self.assertIn("!dailyCommandHealthOk && (dailyCommandP0ReadbackPending || Boolean(error) || !dailyCommandHealthChecked)", source)
