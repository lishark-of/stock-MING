...
class CommandCenterHardRiskPacketTests(unittest.TestCase):
    def test_missing_cache_returns_waiting_manual_packet(self):
        ...
        self.assertEqual(packet["capability_state"], "requires_manual_refresh")
        self.assertEqual(packet["status_label"], "需要手动刷新")
        self.assertEqual(packet["recovery_state"], "waiting")
        self.assertEqual(packet["risk_state"], "待验证")
        self.assertEqual(packet["packet_role"], "A股公告/硬风险证据")
        ...
        self.assertIn("缺少公告/硬风险", packet["decision_guardrail"])
    ...
    def test_normalizes_a_share_professional_hard_risk_cache(self):
        ...
        self.assertIn("硬风险状态：风险线索存在", packet["evidence_summary"])
        self.assertIn("风险等级：high", packet["evidence_summary"])
        self.assertIn("风险线索", packet["evidence_summary"])
        self.assertIn("硬风险未排除前不加仓", packet["action_hint"])
        self.assertIn("不能加仓", packet["decision_guardrail"])
        evidence_by_key = {item["key"]: item for item in packet["evidence_items"]}
        self.assertEqual(evidence_by_key["risk_state"]["value"], "风险线索存在")
        self.assertEqual(evidence_by_key["risk_items"]["status"], "已回流")
    ...
    def test_permission_denied_is_conservative(self):
        ...
        self.assertEqual(packet["data_status"], "missing")
        self.assertEqual(packet["capability_state"], "failed")
        self.assertEqual(packet["status_label"], "调用失败")
        ...
        self.assertIn("调用失败", packet["evidence_summary"])
        self.assertIn("接口权限", packet["action_hint"])
        self.assertIn("不能把数据缺口写成无风险", packet["decision_guardrail"])
