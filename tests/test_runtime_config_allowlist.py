...
class RuntimeConfigAllowlistTests(unittest.TestCase):
    ...
    def test_desktop_launcher_does_not_override_runtime_config_defaults(self):
        ...
        self.assertIn("bootstrap_status_ready", text)
        self.assertIn("wait_for_bootstrap_status", text)
        self.assertIn("command_center_3_bootstrap_runtime_mode_packet", text)
        self.assertIn("command_center_bootstrap_runtime_mode.v1", text)
        self.assertIn("Bootstrap check: /api/bootstrap/status must return", text)
