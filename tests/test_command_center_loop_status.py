...
class CommandCenterLoopStatusTests(unittest.TestCase):
    ...
    def test_old_workspace_packet_bridge_blocks_loop_when_legacy_packets_are_missing(self):
        ...
        self.assertEqual(view_model["recovery_queue"]["title"], "决策闭环恢复队列")
        self.assertEqual(view_model["recovery_queue"]["visible_count"], 2)
        self.assertEqual(view_model["recovery_queue"]["items"][0]["loop_label"], "旧能力回流")
        self.assertEqual(view_model["recovery_queue"]["items"][0]["toolbox_entry"], "高级工具箱 / 融资 ETF")
        self.assertEqual(view_model["recovery_queue"]["items"][0]["external_call_policy"], "navigation_only")
        self.assertIn("手动恢复", view_model["recovery_queue"]["summary"])
        self.assertIn("只负责导航", view_model["recovery_queue"]["safe_mode_text"])
