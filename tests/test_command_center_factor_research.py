...
class CommandCenterFactorResearchTests(unittest.TestCase):
    ...
    def test_factor_test_lab_scaffold_declares_research_metrics_without_external_calls(self):
        ...
        self.assertTrue(packet["quality_summary"]["all_result_states_are_research_only"])
        self.assertTrue(packet["quality_summary"]["research_pass_is_not_trade_signal"])
        ...
        self.assertEqual(packet["acceptance_contract"]["scope"], "light_research_state_acceptance_not_full_market_validation")
        ...
        self.assertFalse(packet["acceptance_contract"]["cache_api_external_calls"])
        self.assertFalse(packet["acceptance_contract"]["react_render_external_calls"])
        state_rows = {row["research_state"]: row for row in packet["state_transition_rows"]}
        self.assertEqual(
            set(state_rows),
            {"research_pass", "watchlist", "disabled", "invalid", "not_enough_data"},
        )
        for row in state_rows.values():
            self.assertEqual(row["governance_state"], "research_only")
            self.assertFalse(row["allow_evidence_effects"])
            self.assertFalse(row["allow_strategy_trace"])
            self.assertFalse(row["allow_core_action"])
            self.assertFalse(row["allow_automatic_trade"])
        ...
        self.assertIn("automatic_trade", state_rows["research_pass"]["blocked_surfaces"])
        self.assertEqual(packet["call_ledger"][0]["api"], "local_factor_test_lab_scaffold")
        self.assertEqual(packet["call_ledger"][0]["state_transition_count"], 5)
    ...
    def test_factor_test_lab_classifies_supplied_metrics_without_action_leakage(self):
        ...
        self.assertIn("research_pass", packet["acceptance_contract"]["observed_states"])
        self.assertIn("invalid", packet["acceptance_contract"]["observed_states"])
        self.assertFalse(packet["acceptance_contract"]["real_small_pool_validation_done"])
        self.assertFalse(packet["acceptance_contract"]["full_market_validation_done"])
        self.assertTrue(packet["acceptance_contract"]["all_result_states_are_research_only"])
        self.assertTrue(packet["acceptance_contract"]["research_pass_is_not_trade_signal"])
        self.assertTrue(packet["acceptance_contract"]["does_not_call_tushare"])
