...
class CandidateRadarQuantProjectionCacheLedgerTests(unittest.TestCase):
    ...
    def test_cross_module_alignment_requires_next_session_source_task_match(self):
        ...
        store.write_packet(
            candidate_service.NEXT_SESSION_PACKET_KEY,
            {
                ...
                "chart_payload": {
                    ...
                    "symbol": "002008.SZ",
                    "source_task_id": "local-old-task",
                    "historical_points": [{"x": "2026-06-10", "price": 10.0}],
        ...
        stale = candidate_service._search_quant_projection_cross_module_alignment_readback(
            "002008.SZ",
            source_task_id="local-current-task",
        )

        self.assertEqual(stale["factor_alignment_state"], "aligned")
        self.assertEqual(stale["next_alignment_state"], "mismatch_source_task")
        self.assertEqual(stale["next_source_task_id"], "local-old-task")
        self.assertFalse(stale["overall_alignment_ready"])
        self.assertFalse(stale["external_calls_triggered"])
        self.assertFalse(stale["tushare_called"])
        self.assertFalse(stale["deepseek_called"])

        next_packet = store.read_packet(candidate_service.NEXT_SESSION_PACKET_KEY)
        next_packet["source_task_id"] = "local-current-task"
        next_packet["chart_payload"]["source_task_id"] = "local-current-task"
        store.write_packet(candidate_service.NEXT_SESSION_PACKET_KEY, next_packet)

        aligned = candidate_service._search_quant_projection_cross_module_alignment_readback(
            "002008.SZ",
            source_task_id="local-current-task",
        )

        self.assertEqual(aligned["next_alignment_state"], "aligned")
        self.assertTrue(aligned["overall_alignment_ready"])
    ...
    def test_deepseek_skips_when_tushare_has_no_non_empty_stock_facts(self):
        self._with_meta_store()
        self._with_env(TUSHARE_TOKEN="REAL_TUSHARE_SECRET_VALUE", DEEPSEEK_API_KEY="REAL_DEEPSEEK_SECRET_VALUE")
        clear_task_statuses_for_tests(clear_persisted=True)
        self._with_snapshot_cache(
        ...
        def fake_run_tushare_refresh_task(payload, **_kwargs):
            return {
                "task_id": "fake-tushare-trade-cal-only-ledger",
                "status": "success",
                "current_step": "tushare_refresh_partial_empty_stock_facts_safe",
                "call_ledger": [
                    {
                        "api": api,
                        "request_params_safe": {
        ...
        self.assertEqual(receipt["provider_api_success_count"], 1)
        self.assertTrue(receipt["deepseek_model_ledger_recorded"])
        ...
        self.assertEqual(model_ledger["status"], "skipped_missing_facts")
        self.assertEqual(model_ledger["model_call_status"], "skipped_missing_facts")
        self.assertFalse(model_ledger["raw_prompt_stored"])
        self.assertFalse(model_ledger["raw_output_stored"])
        self.assertEqual(explanation["status"], "skipped_missing_facts")
        self.assertEqual(explanation["source"], "safe_degraded_status")
        self.assertEqual(explanation["error_message_safe"], "skipped_missing_facts")
        self.assertEqual(lineage["facts_package_status"], "partial_provider")
        self.assertTrue(lineage["deepseek_skipped_missing_facts"])
    ...
    def test_degraded_tushare_result_retains_previous_last_good_current_result(self):
        ...
        self.assertEqual(summary["latest_task_id"], lineage["task_id"])
        self.assertEqual(summary["latest_task_scope_hash"], lineage["scope_hash"])
        self.assertEqual(summary["latest_task_provider_call_ledger_ids"], lineage["provider_call_ledger_ids"])
        self.assertEqual(summary["latest_task_input_packet_keys"], lineage["input_packet_keys"])
        self.assertEqual(summary["latest_task_output_packet_keys"], lineage["output_packet_keys"])
        self.assertEqual(summary["latest_task_data_date"], lineage["data_date"])
        self.assertEqual(summary["latest_task_freshness_state"], lineage["freshness_state"])
        self.assertEqual(summary["latest_task_model_ledger_id"], lineage["model_ledger_id"])
        self.assertEqual(summary["degraded_task_id"], lineage["task_id"])
        self.assertEqual(summary["degraded_scope_hash"], lineage["scope_hash"])
        self.assertEqual(summary["degraded_provider_call_ledger_ids"], lineage["provider_call_ledger_ids"])
        self.assertEqual(summary["degraded_input_packet_keys"], lineage["input_packet_keys"])
        self.assertEqual(summary["degraded_output_packet_keys"], lineage["output_packet_keys"])
        self.assertEqual(summary["degraded_data_date"], lineage["data_date"])
        self.assertEqual(summary["degraded_freshness_state"], lineage["freshness_state"])
        self.assertEqual(summary["degraded_model_ledger_id"], lineage["model_ledger_id"])
        self.assertTrue(summary["degraded_deepseek_skipped_missing_facts"])
        current_lineage = packet["search_quant_current_result_lineage"]
        ...
        self.assertEqual(summary["current_result_data_date"], current_lineage["data_date"])
        self.assertEqual(summary["current_result_freshness_state"], current_lineage["freshness_state"])
    ...
    def test_cache_readback_promotes_latest_result_when_factor_next_same_task_now_aligned(self):
        self._with_meta_store()
        clear_task_statuses_for_tests(clear_persisted=True)
        store = SQLiteMetaStore(candidate_service.SQLITE_META_PATH)
        store.write_packet(
        ...
        self.assertEqual(summary["current_result_symbol"], "000001.SZ")
        ...
        self.assertEqual(
            packet["search_quant_projection_local_factor_next_refresh"]["status"],
            "local_factor_next_echarts_refreshed_from_readback",
        )
        self.assertFalse(packet["external_calls_triggered"])
        self.assertFalse(packet["tushare_called"])
        self.assertFalse(packet["deepseek_called"])
    ...
    def test_deepseek_failure_does_not_block_tushare_factor_next_result(self):
        ...
        def fake_run_tushare_refresh_task(payload, **_kwargs):
            return {
                "task_id": "fake-tushare-success-ledger",
                "status": "success",
                "current_step": "tushare_refresh_completed",
                "call_ledger": [
                    {
        ...
        def failing_model_call(**_kwargs):
            model_call_attempts.append(True)
            raise TimeoutError("unit test model timeout")

        candidate_service.tushare_task_service.run_tushare_refresh_task = fake_run_tushare_refresh_task
        candidate_service._call_quant_projection_deepseek_model = failing_model_call
        self.addCleanup(
            setattr,
            candidate_service.tushare_task_service,
            "run_tushare_refresh_task",
        ...
        self.assertTrue(response["ok"])
        self.assertEqual(len(model_call_attempts), 1)
        cache = self.client.get("/api/candidate-radar/cache").json()
        self.assertTrue(cache["ok"])
        packet = cache["data"]
        receipt = packet["search_quant_provider_model_acceptance_receipt"]
        ...
        self.assertTrue(receipt["deepseek_called"])
        self.assertTrue(receipt["deepseek_safe_degraded"])
        self.assertEqual(receipt["deepseek_safe_failure_mode"], "model_call_failed_or_timeout")
        self.assertEqual(model_ledger["status"], "degraded_model_call_failed_or_timeout")
        self.assertEqual(model_ledger["model_call_status"], "model_call_failed_or_timeout")
        self.assertTrue(model_ledger["deepseek_called"])
        self.assertTrue(model_ledger["external_calls_triggered"])
        self.assertFalse(model_ledger["raw_prompt_stored"])
        self.assertFalse(model_ledger["raw_output_stored"])
        self.assertEqual(explanation["status"], "degraded_model_call_failed_or_timeout")
        self.assertTrue(receipt["factor_refresh_executed"])
        self.assertTrue(receipt["next_session_refresh_executed"])
        self.assertTrue(receipt["echarts_payload_refreshed"])
        self.assertEqual(lineage["freshness_state"], "fresh_provider")
