...
class WorkerCeleryFilesystemRoundtripSmokeTest(unittest.TestCase):
    def test_filesystem_roundtrip_smoke_returns_direct_non_redis_evidence(self) -> None:
        result = subprocess.run(
            [
                sys.executable,
                "scripts/worker_celery_filesystem_roundtrip_smoke.py",
                "--timeout",
                "10",
            ],
            cwd=ROOT,
        ...
        self.assertEqual(payload["schema_version"], "worker_celery_filesystem_roundtrip_smoke.v1")
        self.assertEqual(payload["status"], "celery_filesystem_roundtrip_passed")
        self.assertEqual(
            payload["direct_evidence_layer"],
            "L3_local_celery_filesystem_roundtrip_not_redis",
        )
        ...
        self.assertEqual(payload["returned_payload"]["payload_mode"], "filesystem_roundtrip")
