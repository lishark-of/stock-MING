...
class CommandCenter3TauriPreflightTests(unittest.TestCase):
    ...
    def test_tauri_dev_uses_local_launcher_skip_open_before_window(self):
        config = json.loads(TAURI_CONFIG.read_text(encoding="utf-8"))
        build = config["build"]

        self.assertEqual(build["devUrl"], "http://127.0.0.1:5173")
        self.assertEqual(
            build["beforeDevCommand"],
            "COMMAND_CENTER_3_LAUNCHER_SKIP_OPEN=1 ../scripts/start_command_center_3.command",
        )
        self.assertIn("COMMAND_CENTER_3_LAUNCHER_SKIP_OPEN=1", build["beforeDevCommand"])
        self.assertIn("start_command_center_3.command", build["beforeDevCommand"])
    ...
    def test_command_center_3_launcher_is_local_one_click_and_safe(self):
        ...
        self.assertIn('APP_OPEN_URL="$(safe_display_open_url "$APP_URL")"', source)
        self.assertIn('APP_URL_DISPLAY="$APP_OPEN_URL"', source)
        self.assertIn('LAUNCHER_OPEN_MODE="${COMMAND_CENTER_3_LAUNCHER_OPEN_MODE:-reuse}"', source)
        self.assertIn('APP_URL_REUSE_PREFIX="$(safe_display_url "$APP_URL")', source)
        self.assertIn("focus_existing_browser_tab", source)
        ...
        self.assertIn("launcher reuses an existing local Command Center tab or opens #home by default; user can open #candidates/candidate-radar-search-quant-projection next", source)
        self.assertNotIn("P0 success handoff: after readiness, open #candidates; typing stays silent", source)
        ...
        self.assertIn("system python3 for local URL sanitization only", source)
        self.assertIn("probing URLs, writing logs, opening a browser, creating tasks", source)
        self.assertIn("print_local_port_occupancy", source)
        self.assertIn("本机端口占用摘要（只读 lsof；不杀进程、不探测 URL）", source)
        self.assertIn('"FastAPI:${API_BIND_PORT}" "React/Vite:${VITE_BIND_PORT}"', source)
        self.assertIn("Check-only local port summary: printed current 8710/5173 local LISTEN processes without killing processes or probing URLs.", source)
        self.assertIn("端口占用摘要只展示本机进程名/PID/端口", source)
        ...
        vite_reuse_index = source.index('if vite_command_center_ready "$VITE_URL"; then')
        ...
        self.assertIn("COMMAND_CENTER_3_P0_STABILITY_DWELL_SECONDS", source)
        self.assertIn("P0_STABILITY_DWELL_SECONDS", source)
        self.assertIn("Browser open:", source)
        self.assertIn("Browser open mode: ${LAUNCHER_OPEN_MODE}", source)
        ...
        self.assertIn('elif focus_existing_browser_tab "$APP_URL_REUSE_PREFIX"; then', source)
        self.assertIn("Browser handoff reused existing local tab: ${APP_URL_DISPLAY}", source)
        self.assertIn('if open "$APP_OPEN_URL"; then', source)
        self.assertIn("Browser handoff opened: ${APP_URL_DISPLAY}", source)
        self.assertIn("Browser handoff could not open automatically after local readiness passed", source)
        self.assertIn("verify_p0_startup_stability", source)
        self.assertIn("P0 stability check: waiting ${P0_STABILITY_DWELL_SECONDS}s", source)
        self.assertIn("P0 stability check passed", source)
        self.assertIn("P0 stability check failed", source)
        self.assertIn("P0_STABILITY_READY=0", source)
        self.assertIn("P0 stability ready=${P0_STABILITY_READY}", source)
        self.assertIn("P0 稳定性复核失败", source)
        ...
        self.assertIn("React/Vite 前端：${VITE_URL_DISPLAY} 已返回 Command Center 3.0 HTML；启动器会先复用已有本地页面，找不到时再打开普通首页 ${APP_URL_DISPLAY}", source)
        ...
        self.assertIn("P0 success handoff: after readiness, launcher reuses an existing local Command Center tab or opens #home by default; user can open #candidates/candidate-radar-search-quant-projection next", source)
        self.assertIn("联通后下一步：回首页确认股票代码（#home）；需要详情再打开下一票雷达确认输入区（#candidates/candidate-radar-search-quant-projection）", source)
        self.assertIn("输入本身保持静默；只有确认按钮会创建 Tushare-first POST task", source)
        ...
        self.assertIn("本地入口不会在前后端未联通、Vite 端口不是 Command Center 3.0 页面或 P0 稳定性复核失败时自动打开页面", source)
        self.assertIn('exit 1', source)
        self.assertIn('open "$APP_OPEN_URL"', source)
        self.assertNotIn('open "$APP_URL"', source)
    ...
    def test_command_center_3_launcher_check_only_does_not_start_or_open(self):
        ...
        self.assertIn("system python3 for local URL sanitization only", output)
        self.assertIn("starting React/Vite", output)
        self.assertIn("probing URLs", output)
        self.assertIn("本机端口占用摘要", output)
        self.assertIn("FastAPI port 8710", output)
        self.assertIn("React/Vite port 5173", output)
        self.assertIn("Check-only local port summary: printed current 8710/5173 local LISTEN processes without killing processes or probing URLs.", output)
        ...
        self.assertIn("P0 success handoff: after readiness, launcher reuses an existing local Command Center tab or opens #home by default; user can open #candidates/candidate-radar-search-quant-projection next", output)
        self.assertIn("Browser open mode: reuse", output)
        self.assertIn("Duplicate-tab guard: open mode 'reuse' first focuses an existing Chrome tab", output)
    ...
    def test_p0_post_startup_readback_checklist_is_visible_and_read_only(self):
        ...
        self.assertIn("p0Startup30sQuickReadRows", page)
        self.assertIn("p0_startup_30s_quick_read_rows", page)
        self.assertIn('aria-label="p0 startup 30s quick read"', page)
        self.assertIn("启动后 30 秒速读", page)
        self.assertIn("普通用户打开后先看这张表", page)
        self.assertIn("四段联通和 P0 stability dwell 是否 ready", page)
        self.assertIn('速读项: "1. 页面是否应该打开"', page)
        self.assertIn('速读项: "2. 四段联通"', page)
        self.assertIn('速读项: "3. P0 stability dwell"', page)
        self.assertIn('速读项: "4. 失败先看哪里"', page)
        self.assertIn('速读项: "5. 现在下一步"', page)
        self.assertIn('速读项: "6. 本轮边界"', page)
        self.assertIn("少于 4/4 时按失败段处理；4/4 时回首页确认股票代码。", page)
        self.assertIn("P0_STABILITY_READY=1 + health/bootstrap/preflight/Vite dwell reread", page)
        self.assertIn("stability dwell 只复读本地 health、bootstrap status、desktop preflight cache 和 React/Vite", page)
        self.assertIn("p0_failure_diagnostic_rows + .stock_ming_3/logs + 8710/5173", page)
        self.assertIn("失败定位不读取 token/key，不调用 Tushare/DeepSeek/GitHub，不执行真实交易", page)
        self.assertIn("把 P0 当作本地联通 checkpoint；不要当 release ready 或 14 LTG strict closeout", page)
        self.assertIn("不启用 live_light，不调用 provider/model，不写 cache，不交易，不改 strategy action", page)
        self.assertIn("GET preflight 和 React render 不启动服务、不创建 task", page)
        self.assertIn("输入股票代码保持静默；确认按钮之前不调用 provider/model", page)
        ...
        self.assertIn("只切换到今日作战台首页；输入代码不外联，点击确认才创建 Tushare-first POST task；下一票雷达保留为详情入口", page)
        ...
        self.assertIn("不让旧 cache 覆盖当前状态", page)
        ...
        self.assertIn("真正的 Tushare-first 工作仍要点首页或下一票雷达的确认按钮", health)
        ...
        self.assertLess(page.index("启动后 30 秒速读"), page.index("P0 可继续闸门"))
        self.assertLess(page.index("P0 可继续闸门"), page.index("下一步："))
        self.assertLess(page.index("P0 可继续闸门"), page.index("<summary>开发 / 审计详情</summary>"))
    ...
    def test_p0_quick_action_handoff_requires_confirm_button(self):
        ...
        self.assertIn("首页确认卡", handoff_rows[1]["下一步"])
        self.assertIn("首页确认卡", quick_rows[1]["用户下一步"])

        for row in handoff_rows + quick_rows:
            self.assertEqual(row["frontend_backend_auto_link_scope"], "local_fastapi_only")
            self.assertIn("四段 ready 后主入口切到 #home", row["P0交接证据"])
            self.assertIn("#candidates/candidate-radar-search-quant-projection", row["P0交接证据"])
