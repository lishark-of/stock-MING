...
class CommandCenter3TauriPreflightTests(unittest.TestCase):
    ...
    def test_p0_post_startup_readback_checklist_is_visible_and_read_only(self):
        ...
        self.assertIn("少于 4/4 时按失败段处理；4/4 时进入下一票雷达确认输入区。", page)
        ...
        self.assertIn("普通用户先看这个闸门：四段联通 ready 才去下一票雷达", page)
        ...
        self.assertIn("只切换到下一票雷达；输入代码不外联，点击确认才创建 Tushare-first POST task；链接直达确认输入区", page)
    ...
    def test_p0_quick_action_handoff_requires_confirm_button(self):
        ...
        for row in handoff_rows + quick_rows:
            ...
            self.assertIn("四段 ready 后只切换到 #candidates/candidate-radar-search-quant-projection", row["P0交接证据"])
            self.assertIn("搜票量化推演确认输入区", row["P0交接证据"])
