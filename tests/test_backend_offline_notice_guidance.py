...
class BackendOfflineNoticeGuidanceTests(unittest.TestCase):
    ...
    def test_api_client_falls_back_only_to_local_fastapi_base(self):
        ...
        self.assertIn("async function fetchLocalApi", client)
        self.assertIn("return await fetch(url, { ...init, signal: controller.signal })", client)
        self.assertIn("fetchLocalApi(`${apiBase}${path}`", client)
        self.assertNotIn("fetch(`${apiBase}${path}`", client)
