...
class CommandCenterDisciplinePacketTests(unittest.TestCase):
    def test_missing_cache_returns_waiting_packet(self):
        ...
        self.assertEqual(packet["packet_role"], "交易纪律/回测证据")
        self.assertEqual(packet["verification_status"], "待验证")
        self.assertIn("待手动运行", packet["evidence_summary"])
        self.assertIn("手动运行回测", packet["action_hint"])
        self.assertIn("缺少回测缓存", packet["decision_guardrail"])
    ...
    def test_cached_backtest_builds_ready_packet(self):
        ...
        self.assertIn("纪律边界", packet["evidence_summary"])
        self.assertIn("胜率 62%", packet["evidence_summary"])
        self.assertIn("最大回撤 12%", packet["evidence_summary"])
        self.assertIn("纪律边界", packet["action_hint"])
    ...
    def test_reduce_signal_or_high_drawdown_is_conservative(self):
        ...
        self.assertEqual(drawdown_packet["verification_status"], "阻断决策")
        self.assertIn("纪律边界：降风险", drawdown_packet["evidence_summary"])
        self.assertIn("先降低暴露", drawdown_packet["action_hint"])
        self.assertIn("不能加仓", drawdown_packet["decision_guardrail"])
    ...
    def test_existing_packet_is_preserved_without_mutating_input(self):
        ...
        self.assertIn("回测缓存", packet["decision_guardrail"])
