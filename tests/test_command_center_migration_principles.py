...
class CommandCenterMigrationPrincipleDocsTests(unittest.TestCase):
    def test_long_term_goals_record_no_blind_streamlit_copy_policy(self):
        ...
        self.assertIn("keep the new mode-layered `live_light` line in docs/config/runtime-mode wording first", text)
    ...
    def test_app_migration_plan_records_no_blind_copy_and_audit_gate(self):
        ...
        self.assertIn("The checkpoint contract also fixes the current priority order as", text)
        self.assertIn("`runtime_mode_layering_docs_config_first_slice`", text)
        self.assertIn("keeps the current round focused on wording, config vocabulary, example config, contracts, and focused tests", text)
