...
class CommandCenterMigrationPrincipleDocsTests(unittest.TestCase):
    ...
    def test_candidate_radar_quant_projection_button_shows_disabled_reason(self):
        ...
        self.assertIn("按钮不可用原因：先输入股票代码；输入本身不会启动数据链", text)
        self.assertIn("按钮已启用：确认后生成 ${quantProjectionSymbolValidation.normalized} 的本地投研结果", text)
    ...
    def test_stock_quant_projection_demotes_engineering_audit_from_ordinary_first_view(self):
        ...
        self.assertIn("输入代码并点击生成 3.0 量化推演；这个链接只切换本地页面，不创建后台流程", text)
        self.assertIn("工程审计明细默认收起；完整数据接口、模型、记录和配置状态", text)
        self.assertIn('href="#audit"', text)
        self.assertIn('href="#settings"', text)
    ...
    def test_app_migration_plan_records_no_blind_copy_and_audit_gate(self):
        ...
        self.assertIn("The checkpoint contract fixes the current user-usable priority order as", text)
        self.assertIn("`p0_one_click_startup_frontend_backend_connection`", text)
        self.assertIn("`p1_confirmed_stock_code_tushare_first_data_chain`", text)
        self.assertIn("`p2_small_data_write_cache_ledger_packet`", text)
        self.assertIn("`p3_interpretable_candidate_radar_quant_projection_next_session_map`", text)
        self.assertIn("`p4_demote_engineering_audit_noise_from_ordinary_pages`", text)
        self.assertIn("`p5_deepseek_governed_executor_separate_nonblocking`", text)
        self.assertIn("`p6_return_to_14_ltg_direct_evidence_strict_closeout`", text)
        self.assertIn("future_migration_slices_follow_p0_to_p6_usable_path_or_name_blocker_exception", text)
        self.assertIn("runtime-mode docs/config wording remains a supporting guard", text)
        self.assertIn("not an active priority item that can displace P0/P1/P2/P3 usability", text)
        self.assertIn("a release blocker rather than the daily user-usable priority driver", text)
        self.assertNotIn("The checkpoint contract fixes the current release-first priority order as", text)
