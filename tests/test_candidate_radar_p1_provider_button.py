...
class CandidateRadarP1ProviderButtonTests(unittest.TestCase):
    def test_provider_acceptance_button_is_explicit_tushare_first_after_execution_request(self):
        ...
        self.assertIn("include_deepseek: true", provider_slice)
        self.assertIn('requested_by: "candidate_radar_page"', provider_slice)
        self.assertIn("const launchQuantProjectionDeepSeekFailureAcceptance = () =>", provider_slice)
        self.assertIn('deepseek_failure_mode_for_acceptance: "missing_server_key"', provider_slice)
        self.assertIn('requested_by: "candidate_radar_page_deepseek_failure_acceptance"', provider_slice)
        self.assertIn("确认 Tushare-first 补证", execution_request_slice)
        self.assertIn("验证 DeepSeek 安全降级", execution_request_slice)
        ...
        self.assertIn("DeepSeek 只读解释可安全降级", execution_request_slice)
        self.assertIn("同一个 scope hash 下先保留 Tushare / Factor / Next 事实结果", execution_request_slice)
        self.assertIn("missing_server_key 降级账本", execution_request_slice)
        ...
        self.assertIn("const desktopP0CurrentReadbackReady =", source)
        self.assertIn("const quantProjectionLocalAppReady =", source)
        self.assertIn("candidateRadarCacheGetReadable &&", source)
        self.assertIn("const desktopP0ConnectionEvidenceReady =", source)
        self.assertIn("desktopP0CurrentReadbackReady;", source)
        ...
        self.assertIn("p0_connection_evidence_ready: desktopP0ConnectionEvidenceReady || quantProjectionLocalAppReady", source)
        ...
        self.assertIn("只有确认按钮可创建 Tushare-first task；DeepSeek 只读解释可安全降级，不交易", source)
        ...
        self.assertIn("本地 FastAPI/cache 尚未接上：先刷新本地回放或回一键启动预检；输入保持静默，确认按钮不可用。", source)
        ...
        self.assertIn("<summary>高级：DeepSeek 解释治理</summary>", ordinary_quant_slice)
        self.assertIn("按钮门控模型解释何时可回放或安全降级", ordinary_quant_slice)
        self.assertIn("不作为数据源，不阻塞 P1/P2/P3 本地回放", ordinary_quant_slice)
        ...
        self.assertIn("何时允许 DeepSeek 解释、当前是否安全降级、以及只能写安全摘要", ordinary_quant_slice)
        ...
        self.assertNotIn("验证 DeepSeek 安全降级", ordinary_quant_slice)
        ...
        self.assertIn("p0_ready: quantProjectionConfirmGateReady", submit_slice)
        ...
        self.assertIn("p0_connection_evidence_ready: desktopP0ConnectionEvidenceReady || quantProjectionLocalAppReady", submit_slice)
