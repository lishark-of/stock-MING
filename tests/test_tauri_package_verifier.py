...
class TauriPackageVerifierTests(unittest.TestCase):
    ...
    def _write_evidence(self, root: Path, *, screenshot_bytes: bytes = b"offline-ui") -> str:
        module = _load_module()
        package_root = root / "desktop_runtime"
        package_root.mkdir(parents=True)
        app_path = root / "fake.app"
        dmg_path = root / "fake.dmg"
    ...
    def test_changed_disk_artifact_blocks_promotion(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            self._write_evidence(root)
            (root / "fake.dmg").write_bytes(b"tampered disk image")
            ...
            self.assertIn("dmg_disk_hash_mismatch", result["blockers"])
