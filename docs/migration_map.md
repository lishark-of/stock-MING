...
| Factor Quant Hub | `command_center_factor_research.py`, `app.py` | `GET /api/factor-quant/cache`, `POST /api/factor-quant/refresh-data`, `POST /api/factor-quant/run-light` | `FactorQuantHub.tsx` | cache GET 优先读取 SQLite 持久化 packet，snapshot/local builder 兜底；run-light 本地 light 计算并写入 factor_values Parquet；refresh-data 已按钮门控接入 Tushare `daily/daily_basic/moneyflow`，扩展接口仍需逐项验收 | 是，run-light 已本地 pipeline |
