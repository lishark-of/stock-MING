...
|---|---|---|---|---|---|
| 次日操作图谱 | `command_center_next_session_projection.py`, `app.py` | `GET /api/next-session/cache`, `POST /api/next-session/generate` | `NextSessionMap.tsx` | cache GET 不重算；generate 后续重算 | 是，当前 stub |
| Factor Quant Hub | `command_center_factor_research.py`, `app.py` | `GET /api/factor-quant/cache`, `POST /api/factor-quant/refresh-data`, `POST /api/factor-quant/run-light` | `FactorQuantHub.tsx` | cache GET 不重算；refresh/run 后续重算 | 是，当前 stub |
| A 股事实血缘 | `command_center_evidence_summary.py`, `command_center_*_packet.py` | 后续 `/api/packets/{packet_key}` 与 fact refresh task | `CommandCenterHome.tsx` / 后续证据页 | cache 不重算 | 后续任务化 |
...
| 产业链瓶颈扫描 | `command_center_analysis_methods.py`, `analysis_engine.py`, `app.py` | `GET /api/chokepoint/cache`, `POST /api/chokepoint/run` | `ChokepointScan.tsx` | run 后续调用 DeepSeek/本地评分 | 是，当前 stub |
| Serenity 方法雷达 | `command_center_serenity_method_radar.py`, `app.py` | `GET /api/serenity/cache`, `POST /api/serenity/github-probe` | `SerenityMethodRadar.tsx` | cache 不重算；probe 后续外联 | 是，当前 stub |
