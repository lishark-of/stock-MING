...
| Storage datasets | `storage/parquet_store.py`, `storage/duckdb_store.py` | `GET /api/storage`, `GET /api/storage/catalog`, `GET /api/storage/{dataset}`, `POST /api/storage/artifact-hygiene/dry-run`, `POST /api/storage/schema-validation/dry-run`, `POST /api/storage/partition-migration/dry-run`, `POST /api/storage/compaction/dry-run`, `POST /api/storage/cache-ttl/dry-run` | `CommandCenterHome.tsx`, `StorageOverview.tsx` | GET 只读 Parquet/DuckDB 状态、数据集目录、DuckDB query service policy、dataset version policy、schema migration preflight 和 artifact hygiene；DuckDB policy 展示 canonical path、支持过滤、limit guard、参数绑定和前端不直接读 DataFrame 边界；dataset version policy 只展示 declared version、manifest 路径和物理验证边界，不写 `_dataset_versions.json`、不把声明版本当生产验收；schema migration preflight 只展示 schema version、required columns、primary key 与 partition expectation，不做物理验证、不写 Parquet；schema validation dry-run 只读 Parquet schema metadata，对比物理列与合同列，不读取行 payload、不迁移、不外联；partition migration dry-run 只生成 ready/blocked/missing 分区迁移计划，不写 partitioned Parquet；compaction dry-run 只生成 ready/not-needed/missing 压缩预检清单，不重写 Parquet；cache TTL dry-run 只读取本地文件 metadata，输出 fresh/stale/missing 与 refresh-recommended 清单，不补数、不调用外部源、不写 Parquet；artifact POST dry-run 只生成本地清理预检任务和候选清单，不删除文件、不读取 payload、不外联 | 是，artifact cleanup dry-run、schema validation dry-run、partition migration dry-run、compaction dry-run 与 cache TTL dry-run 为本地按钮任务；真实 cleanup、真实 dataset version manifest、真实 schema migration、真实 partition migration、真实 compaction、typed projection/pagination 与真实 refresh scheduling 仍未接入 |
