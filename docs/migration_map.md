...
| 数据健康时间线 / Provider 诊断 | `command_center_data_health_ledger.py`, `command_center_data_capability_console.py`, `app.py` | `GET /api/data-health/cache` | `DataHealthTimeline.tsx` | cache GET 只读健康时间线、provider 诊断、能力矩阵、恢复动作、缺口报告和 LTG-01 freshness 验收矩阵；不 ping provider、不刷新数据 | 后续 provider probe 任务化；当前不提供 POST |
...
`/api/data-health/cache` 已接入数据健康时间线 / Provider 诊断只读迁移：读取本地 `data_health_timeline`、`provider_data_capability_cockpit`、`a_share_capability_matrix`、`data_health_ledger`、`data_gap_report` 和数据健康恢复动作，并输出 LTG-01 freshness 验收矩阵。该矩阵覆盖盘前、盘中、收盘集合竞价、16:30 后、周末/节假日、`trade_cal` 缺失、provider delay grace 与 stale/expired/historical/unknown 的 expected trade date 和 research-only 边界；它是本地合同，不代表真实长窗口 `trade_cal` 验收已完成。不 ping Tushare/AkShare/yfinance/Supabase，不调用 DeepSeek/GitHub，不刷新数据，不执行真实交易，不修改持仓或 `strategy_execution_packet.action`。
