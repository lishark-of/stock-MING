...
| Storage datasets | `storage/parquet_store.py`, `storage/duckdb_store.py` | `GET /api/storage`, `GET /api/storage/catalog`, `GET /api/storage/{dataset}`, `POST /api/storage/artifact-hygiene/dry-run`, `POST /api/storage/schema-validation/dry-run`, `POST /api/storage/partition-migration/dry-run` | `CommandCenterHome.tsx`, `StorageOverview.tsx` | GET 只读 Parquet/DuckDB 状态、数据集目录、schema migration preflight 和 artifact hygiene；schema migration preflight 只展示 schema version、required columns、primary key 与 partition expectation，不做物理验证、不写 Parquet；schema validation dry-run 只读 Parquet schema metadata，对比物理列与合同列，不读取行 payload、不迁移、不外联；partition migration dry-run 只生成 ready/blocked/missing 分区迁移计划，不写 partitioned Parquet；artifact POST dry-run 只生成本地清理预检任务和候选清单，不删除文件、不读取 payload、不外联 | 是，artifact cleanup dry-run、schema validation dry-run 与 partition migration dry-run 为本地按钮任务；真实 cleanup、真实 schema migration 与真实 partition migration 仍未接入 |
