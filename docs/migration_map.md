...
| Factor Quant Hub | `command_center_factor_research.py`, `app.py` | `GET /api/factor-quant/cache`, `POST /api/factor-quant/refresh-data`, `POST /api/factor-quant/run-light` | `FactorQuantHub.tsx` | cache GET 优先读取 SQLite 持久化 packet，snapshot/local builder 兜底；run-light 本地 light 计算并写入 factor_values Parquet；Factor Test Lab 暴露 `research_pass/watchlist/disabled/invalid/not_enough_data` 状态验收合同，所有状态均为 research-only；GET cache 附加 `factor_values` DuckDB 查询消费合同，展示 typed projection、query result contract、cursor page_info 和本地查询血缘，但不把 query rows 当作生产 IC 验收或交易信号；Factor universe 暴露 `current_target/watchlist/custom_pool/full_pool` 研究合同，当前仅 `current_target` light pipeline 已实现，watchlist/custom/full-pool 仍需未来 task/worker；refresh-data 已按钮门控接入 Tushare `daily/daily_basic/moneyflow`，并可按 payload 选择 `trade_cal` 与 margin/limit/cyq/hard-risk 扩展接口进入审计验证；`trade_cal` 可落 Parquet，其他扩展接口只写验证状态，不伪装落盘 | 是，run-light 已本地 pipeline |
