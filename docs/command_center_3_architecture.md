...
- `GET /api/data-health/cache` 现在同时输出 LTG-01 freshness 验收矩阵，把盘前、盘中、收盘集合竞价、16:30 后、周末/节假日、`trade_cal` 缺失、provider delay grace 与 stale/expired/historical/unknown 的 expected trade date、research-only 边界和 action 隔离写成可审计表格。该矩阵只读、cache-only，不调用 Tushare/DeepSeek/GitHub，也不代表真实长窗口 `trade_cal` 验收已经完成。
