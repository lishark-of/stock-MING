...
- React 已加入受控动效清晰度层：route/card/metric/task 使用 CSS 有限时长动效，`StateClarityRail` 将 cache/page state、task receipt、task status 与 Candidate radar 状态显示为 accepted/running/blocked/done 等视觉边界，Next-session ECharts 使用短时更新动效并尊重 `prefers-reduced-motion`，Candidate radar primary result cluster 只基于已有 cache 状态做视觉分组。`/api/audit/cache` 还输出 `motion_clarity_audit` 和 `motion_clarity_rows`，静态审计 motion tokens、finite keyframes、reduced-motion fallback、chart/radar clarity scopes、layout containment、no timer/RAF loop 和 provider-call 边界。该层不使用定时器或 `requestAnimationFrame`，不触发外部调用、不重新计算候选/图谱、不修改 packet、价格、持仓、`operation_zones` 或 `strategy action`；`static_ready` 不等于浏览器视觉验收完成。
