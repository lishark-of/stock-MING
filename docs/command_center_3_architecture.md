...
- `/api/audit/cache` 同时输出 `release_gate_readiness_audit`、`release_gate_readiness_rows` 和 `.github/workflows` 本地静态清单，用来审计 `scripts/push_gate_3_0.sh` 是否包含 Python unittest、desktop build、3.0 smoke、Data Health freshness contract、Tushare acceptance contract、Factor Test Lab contract、DeepSeek governance contract、Candidate Radar contract、Storage contract、Worker contract、motion QA contracts、diff check、secret scan、structured keyword review、artifact scan、可选 report、clean worktree、no push/no add dot/no external/no trade 边界。`scripts/data_health_freshness_contract.py` 只调用本地 Data Health cache builder，防止 LTG-01 freshness 矩阵、fixture、provider runbook、decision-surface audit、producer-coverage audit 误称 provider-backed 完成或丢失 no-provider/no-trade 边界；它不是 provider acceptance run。`scripts/deepseek_governance_contract.py` 只读取本地 Factor Quant Hub cache、sanitizer、response-format review 和 task catalog，防止 manual/default-off governance、JSON stability audit、response-format review、parse-failed discard 或 button-gated task catalog 被误称为 provider benchmark、provider response-format enforcement、retry/repair readiness、auto-after-task production readiness 或 production DeepSeek explanation completion。`scripts/candidate_radar_contract.py` 只读取本地 Candidate Radar cache/service 合同，防止快扫、full-pool plan、deep-scan plan、no-feature-loss QA、replacement gap triage 或 result-delta clarity 被误称为生产雷达替代、旧雷达退场、浏览器性能验收或买入指令。`scripts/storage_contract.py` 只读取本地 Storage cache 和 dry-run packet builder，防止 schema/version preflight、schema validation、partition migration、compaction、cache TTL、DuckDB query policy 或 artifact cleanup review 被误称为物理迁移、provider refresh、delete cleanup 或生产存储完成。`scripts/worker_contract.py` 只读取本地 Worker cache 和 task catalog，防止 dispatch plan、blocker audit、healthcheck QA、activation review 或 scheduler 边界被误称为 process start、broker reachability、healthcheck execution、scheduler enablement 或 production worker completion。`scripts/secret_keyword_review_contract.py` 将普通 keyword 命中分类为结构化计数并抑制原始源码行输出；高风险 secret-like value 仍由 gate 阻断。`.github/workflows/command-center-3-push-gate.yml` 现在静态镜像该本地 gate：CI 创建 `.venv`、安装 desktop dependencies，并以 `PYTHON_BIN=.venv/bin/python` 运行脚本。该审计只读本地脚本和 workflow，不运行 gate、不调用 GitHub API；`local_gate_ready=true` 与 `ci_mirror_ready=true` 不是远端 check 结果，`false_positive_allowlist_review_pending` 仍保留为长期缺口。
...
- React 已加入受控动效清晰度层：route/card/metric/task 使用 CSS 有限时长动效，导航 active route 通过 `aria-current`、`data-route-active` 和一次性 context sweep 明确当前位置，状态徽标通过 `data-status-tone` 与小圆点强化状态可见性，`StateClarityRail` 将 cache/page state、task receipt、task status 与 Candidate radar 状态显示为 accepted/running/blocked/done 等视觉边界。cache loading/error/empty、task phase panel 和 task receipt 还共享 `state_change_confirmation` / `cc-phase-confirm` 有限确认线索，让刷新和任务状态变化更容易看见。`scripts/motion_viewport_qa_contract.py` 固定 LTG-14 的 route/viewport QA 矩阵并接入 push gate，但它只做本地静态合同，不运行浏览器；`scripts/motion_browser_qa_runbook.py` 进一步固定未来浏览器 QA 的手动 FastAPI/Vite 启动顺序、本地 URL、截图/trace artifact policy、视觉验收标准、reduced-motion pass 和性能预算，同样接入 push gate，但不打开浏览器、不写截图、不证明验收完成。Next-session ECharts 使用短时更新动效并尊重 `prefers-reduced-motion`，Candidate radar primary result cluster 只基于已有 cache 状态做视觉分组，并展示 `result_delta_clarity_contract` 里的候选数量、截断、跳过原因、provider gap、freshness、scan mode 和本地 previous-cache diff 线索。`/api/audit/cache` 还输出 `motion_clarity_audit`、`motion_clarity_rows`、`motion_production_qa_contract`、`motion_production_qa_rows`、`motion_browser_qa_runbook_contract`、`motion_browser_qa_runbook_rows` 和 `motion_browser_qa_matrix_rows`，静态审计 motion tokens、finite keyframes、navigation/status context cues、task/cache phase confirmation cues、motion viewport QA contract、browser QA runbook、reduced-motion fallback、chart/radar clarity scopes、layout containment、no timer/RAF loop、provider-call 边界，以及生产动效所需的视觉 QA / 性能 trace pending 状态。该层不使用定时器或 `requestAnimationFrame`，不触发外部调用、不重新计算候选/图谱、不修改 packet、价格、持仓、`operation_zones` 或 `strategy action`；`static_ready`、`local_motion_qa_ready`、`local_runbook_ready` 和 result-delta 本地 diff 状态都不等于浏览器视觉验收完成或生产动效完成。
...
- `scripts/storage_contract.py` 已接入本地 push gate，用本地 cache/dry-run 合同固定 LTG-05 边界；它不运行 schema migration、不写 dataset version manifest、不执行 partition migration、不重写 Parquet、不刷新 TTL provider、不删除 artifact，也不证明生产存储完成。
