...
- `GET /api/data-health/cache` 现在同时输出 LTG-01 freshness 验收矩阵、synthetic 长窗口样本验收、本地 `trade_cal` Parquet 物理验收、`trade_cal_provider_acceptance_runbook`、`current_evidence_freshness_qa_contract` 和 `current_evidence_decision_surface_audit`。矩阵把盘前、盘中、收盘集合竞价、16:30 后、周末/节假日、`trade_cal` 缺失、provider delay grace 与 stale/expired/historical/unknown 的 expected trade date、research-only 边界和 action 隔离写成可审计表格；current-evidence QA 进一步固定 expected trade date、data date 对齐、freshness state eligibility、历史样本隔离、provider-backed acceptance pending 和 decision-surface isolation；decision-surface audit 只读本地 snapshot 中可见的 `composite_score`、`support_factors`、`evidence_preview`、`next_session_bridge.preview` 和 `strategy_action` 字段，发现 research-only 当前证据仍有 score/support/preview 值时列 blocker，缺失字段只标 `not_observed`，不当作生产证明。synthetic sample 使用实际 freshness gate 覆盖长窗口、节假日簇、长周末和缺今日行；local physical validation 只读已有 storage/DuckDB cache，检查 schema、日期窗口、开闭市行、当前日期覆盖、latest completed trading day 与 freshness gate context，但即使本地物理验收通过也不会把 provider-backed acceptance 标成完成。provider runbook 固定后续真实 `trade_cal` 验收的显式 POST task、安全 payload、call_ledger、长窗口、schema、failure modes、artifact promotion 和 current-evidence 边界。它们都不调用 Tushare/DeepSeek/GitHub；缺失或覆盖不足时只输出 blocker，不刷新 provider、不写文件、不重新评分、不过滤 packet、不修改 `strategy action`。
