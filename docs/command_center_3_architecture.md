...
- `GET /api/candidate-radar/cache`、`POST /api/candidate-radar/scan-quick` 与 `POST /api/candidate-radar/full-pool-plan` 输出下一票雷达 coverage detail 与 scan acceptance：`coverage_detail_summary`、`scan_execution_summary`、`scan_acceptance_rows`、`provider_coverage_rows` 和 `degraded_mode_rows` 会把 universe size、provider blocked、stale input、missing provider data、freshness、local pool、full-pool boundary 与 degraded modes 结构化展示。full-pool plan 只写 `full_pool_scan_plan`、stage/filter/required-signal/blocker rows，不扫描全市场、不刷新 provider、不生成买入候选。该层只读本地 snapshot/payload，不在页面渲染时补数、不做 full-pool scan、不调用 Tushare/DeepSeek/GitHub、不把候选分数写入 `strategy action`。
...
- React 已加入首批受控动效清晰度层：route/card/metric/task 使用 CSS 有限时长动效，Next-session ECharts 使用短时更新动效并尊重 `prefers-reduced-motion`，Candidate radar primary result cluster 只基于已有 cache 状态做视觉分组。该层不触发外部调用、不重新计算候选/图谱、不修改 packet、价格、持仓、`operation_zones` 或 `strategy action`。
