...
- `scripts/storage_contract.py` 已接入本地 push gate，用本地 cache/dry-run/review/validate/activation receipt 合同固定 LTG-05 边界；它不运行 schema migration、不把 manifest review/validate 当 writer、不执行 partition migration、不重写 Parquet、不刷新 TTL provider、不删除 artifact，也不证明生产存储完成。
