...
- `GET /api/candidate-radar/cache`、`POST /api/candidate-radar/scan-quick` 与 `POST /api/candidate-radar/full-pool-plan` 输出下一票雷达 coverage detail：`coverage_detail_summary`、`provider_coverage_rows` 和 `degraded_mode_rows` 会把 universe size、provider blocked、stale input、missing provider data 与 degraded modes 结构化展示。full-pool plan 只写 `full_pool_scan_plan`、stage/filter/required-signal/blocker rows，不扫描全市场、不刷新 provider、不生成买入候选。该层只读本地 snapshot/payload，不在页面渲染时补数、不做 full-pool scan、不调用 Tushare/DeepSeek/GitHub、不把候选分数写入 `strategy action`。
