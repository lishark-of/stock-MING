...
The roadmap shorthand for this mode-layered direction is `Mode-layered live-light evidence factory` / `运行模式分层的轻量实时投研证据工厂`. In this plan it is docs/config wording only: it can align the runtime vocabulary, safe config rows, and POST-task evidence boundary, but it cannot replace direct Legacy Bug / UX Audit evidence, promote any old Streamlit workflow to `KEEP`, prove frontend wiring, prove provider/model execution, or become production acceptance evidence.
...
Direct evidence attachments must be safe references: a screenshot reference, redacted reviewer note, or safe log summary. Do not attach raw packet bodies, raw logs, token/key/credential values, unredacted model output, or generated artifacts as Legacy Bug / UX Audit evidence.
...
The first-round direct intake template is also config-owned by `COMMAND_CENTER_LEGACY_AUDIT_CLASSIFICATION_CONTRACT` as `first_round_legacy_bug_ux_audit_collects_direct_problem_statement_not_keep_promotion`. It requires `user_observation`, `legacy_ux_bug_or_patchwork`, `data_lineage_observation`, `replacement_user_path`, `frozen_legacy_path`, `evidence_attachment`, and `keep_promotion_decision`, and it allows only safe screenshot references, redacted reviewer notes, or safe log summaries as attachments. Raw packet bodies, raw logs, token/key/credential values, unredacted model output, and generated artifacts remain forbidden; allowed first-round outcomes are `direct_evidence_intake_pending`, `direct_evidence_observed_redesign_required`, `blocked_by_lineage`, `legacy_debug_retained`, or `retire_confirmed`, never `KEEP`.

`GET /api/migration/status` now exposes that same intake template as `legacy_audit_first_round_intake` and `legacy_audit_first_round_intake_rows`. This is a read-only status surface for the next reviewer: it shows required fields, safe/forbidden attachment sources, focus workflow count, and `KEEP`/ordinary-entry promotion disabled for the first round; it does not create tasks, open Streamlit, call provider/model/GitHub, write cache/config, or become production evidence.
...
The migration plan references `runtime_mode_policy_rows` as the shared config/status vocabulary for this boundary. Those rows must carry `page_open_rule`, `search_submit_rule`, `fastapi_startup_rule`, `search_typing_rule`, `cache_get_rule`, `react_render_rule`, `ledger_rule`, `ordinary_entrance_visibility_rule`, `ordinary_mode_banner_rule`, `configured_switch_rule`, `effective_external_call_rule`, and `production_evidence_rule` so app planning can verify page open and safe searched-symbol submit are mode-specific trigger surfaces; FastAPI startup, search typing, GET cache, and React render remain read-only/non-creating by default; call/model ledger requirements for `manual` or `live_light` external work; ordinary-entry `任务边界` visibility before Settings / Developer / Audit; ordinary mode banner read-only status display rather than task launching or config writing; `configured=true` as operator intent rather than an effective external call; effective external work still requiring mode/task gate plus ledgers/redaction/promotion; and config policy rows remaining non-production evidence.

`page_open_rule` and `search_submit_rule` are config wording, not frontend wiring. `cache_only` uses them to keep page open and confirmed submit cache-only; `manual` keeps page open silent and submit explicit-button/POST-only; `live_light` may only use page open after cache render or a confirmed symbol submit to create/reuse bounded local POST tasks when the effective gates allow it; `live_full` stays reserved with no page-open or search-submit task. These rows must not be read as permission for search typing, React render, FastAPI startup, GET cache, or a visible mode banner to call providers/models or write cache/config.
...
The app plan also treats `runtime_mode_config_current_acceptance_scope`, `runtime_mode_config_current_acceptance_rule`, and `runtime_mode_config_current_acceptance_excludes` as the status-visible guardrail for this docs/config slice. The current scope remains `runtime_mode_vocabulary_config_rows_and_contract_tests_only`; the current rule remains `docs_config_contract_evidence_only_not_live_light_implementation`; the excluded surfaces remain `frontend_autostart_wiring`, `provider_model_executor`, `worker_dispatch`, `cache_write_promotion`, and `production_acceptance`. App planning must not turn those rows into frontend autostart wiring, provider/model execution, worker dispatch, cache-write promotion, or production acceptance evidence.
...
The checkpoint contract fixes the current user-usable priority order as `p0_one_click_startup_frontend_backend_connection`, `p1_confirmed_stock_code_tushare_first_data_chain`, `p2_small_data_write_cache_ledger_packet`, `p3_interpretable_candidate_radar_quant_projection_next_session_map`, `p4_demote_engineering_audit_noise_from_ordinary_pages`, `p5_deepseek_governed_executor_separate_nonblocking`, and `p6_return_to_14_ltg_direct_evidence_strict_closeout`. Future slices must obey `future_migration_slices_follow_p0_to_p6_usable_path_or_name_blocker_exception`; runtime-mode docs/config wording remains a supporting guard for these slices, not an active priority item that can displace P0/P1/P2/P3 usability, Legacy Bug / UX Audit, or ordinary-page audit demotion. `remote_ci_unverified_remains_release_blocker_until_current_green_or_reviewed_logs` keeps release claims blocked until current CI evidence exists, but it is a release blocker rather than the daily user-usable priority driver; `legacy_bug_ux_audit_precedes_major_ordinary_workflow_migration` keeps Streamlit modules from blind promotion, and `ordinary_user_workflow_slices_precede_extra_engineering_scaffold` keeps ordinary-user clarity ahead of extra engineering tables.
