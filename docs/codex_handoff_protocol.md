...
- Priority alignment: name which current migration priority was advanced (`push gate / CI`, `runtime-mode docs/config first slice`, `Legacy Bug / UX Audit`, `Candidate Radar`, searched-symbol -> `生成 3.0 量化推演`, provider/model/cache/pending state, or engineering-audit demotion) and say when a round deliberately advanced none. Runtime-mode docs/config first-slice rounds only align wording, config vocabulary, example config, contracts, or focused tests; they do not authorize a broad one-shot `live_light` implementation or production claim.
