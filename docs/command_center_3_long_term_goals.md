...
11. Keep `worker_runtime_qa_execution_recipe` current so the future runtime QA pass has an ordered checklist before any process starts or broker pings are allowed.
12. Keep `worker_runtime_durable_evidence_recipe` current so production promotion cannot proceed until every direct durable evidence row is collected and reviewed.
13. Add Celery worker execution behind explicit configuration.
14. Add Redis broker configuration and health reporting without cache API pinging Redis.
15. Add retry/cancel/lock behavior for real worker tasks.
16. Keep scheduler default off.
