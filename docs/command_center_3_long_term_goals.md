...
| LTG-04 | Factor 全市场 / 股票池研究 | light mode plus local read-plan, readiness/activation receipts, local rank/zscore sufficiency audit, worker-batch dry-run scope ticket, worker-batch execution recipe, and worker stage-scope manifest; real worker execution pending | watchlist / custom pool / full pool research pipeline | P3 | Large universe runs in task pipeline without blocking UI or entering strategy action. |
...
4. Add batch execution through task pipeline.
5. Add cross-sectional rank, zscore, neutralization, and result summaries.
6. Keep UI as progress/result display only.
