...
| LTG-06 | Worker / Celery / Redis 生产化 | local task fallback, preflight, blocker audit, healthcheck QA contract, readiness/activation receipts, runtime QA execution recipe, runtime evidence stage-scope manifest, and push-gate contract exist | Production-capable worker orchestration with local fallback | P4 | POST returns task_id, worker runs heavy jobs, Redis absence falls back gracefully, scheduler stays off by default, runtime QA evidence is explicit. |
...
11. Add Celery worker execution behind explicit configuration.
12. Add Redis broker configuration and health reporting without cache API pinging Redis.
13. Add retry/cancel/lock behavior for real worker tasks.
14. Keep scheduler default off.
