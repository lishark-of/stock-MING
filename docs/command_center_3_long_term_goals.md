...
7. Keep `production_validation_qa_contract` current until provider-backed validation tasks can prove completion.
8. Keep `factor_test_production_stage_scope_manifest` current whenever provider samples, rolling windows, cost assumptions, neutralization evidence, bias controls, or promotion evidence changes.
