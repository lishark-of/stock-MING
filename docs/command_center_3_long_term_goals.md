...
- Migration Status now lists the LTG-05 local storage receipt sequence as six separate steps: schema acceptance, manifest dry-run, manifest review, confirm-gated manifest write, manifest validate, and physical execution request. This is a progress accelerator for the next safe local button only; it does not turn schema/manifest packets into Parquet migration, compaction, TTL refresh, cleanup delete, provider refresh, or production storage evidence.
