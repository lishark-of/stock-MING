...
Runtime-mode evidence split for the next implementation commits:

| layer | closes | required evidence | not enough |
|---|---|---|---|
| L1 mode contract | The runtime vocabulary and forbidden boundaries are documented. | `cache_only/manual/live_light/live_full` wording, default-off config, GET/render silence, no-trade/no-secret/no-action rules. | A roadmap paragraph alone does not prove task behavior. |
| L2 local bootstrap readiness | The local task shape can be audited before execution. | Safe config rows, task catalog row, staged run plan, rate-limit/dedupe, local call ledger, model-ledger preview, dry-run/preflight receipt. | Skeleton, dry-run, runbook, sanitizer, matrix, or receipt cannot be called provider/model acceptance. |
| L3 live provider/model evidence | `live_light` actually ran bounded external work under user-approved mode. | Real Tushare call ledger for `trade_cal/daily/daily_basic/moneyflow`, optional DeepSeek pro model ledger, browser non-blocking evidence, redaction review, safe failure rows. | Local receipts, fixtures, mocked ledgers, or credential-presence booleans. |
| L4 promotion gate | The feature can be promoted as a reliable app workflow. | Push gate / remote CI, production promotion review, release-safe docs, no secret/artifact ingress, no trading/action mutation. | A green local contract script or one-off manual run. |

The next code commits should follow this split: first keep documentation/governance exact, then add the real bootstrap executor behind explicit local configuration and user-approved task scope, then add search-to-quant projection using the same task boundary, and only then consider DeepSeek pro automatic explanation evidence. Each step must state whether it is L1/L2/L3/L4.
