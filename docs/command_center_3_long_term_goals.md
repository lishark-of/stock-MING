...
- Migration Status now observes the LTG-14 `motion_production_stage_scope_manifest` from the local static motion contract and surfaces it in `ltg_stage_scope_observed_rows` beside LTG-13. This makes the global 14-LTG page show motion stage count, pending count, local source evidence count, visual/performance/CI blockers, and no-provider/no-model/no-trade boundaries without opening a browser, writing QA artifacts, calling GitHub, calling Tushare/DeepSeek, mutating packet values, or completing production motion.
