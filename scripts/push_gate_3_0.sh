...
# A clean CI checkout has no ignored local execution packets, so the storage
# contract may also report the two direct-readback evidence gaps. They are
# expected production-pending blockers, not a contract failure.
run_step "Storage contract" run_local_contract_step "Storage contract" scripts/storage_contract.py "physical_execution_phase_a_is_local_direct_evidence_not_production,physical_durable_evidence_recipe_is_local_pending,partition_migration_execution_has_direct_readback,compaction_execution_has_direct_readback"
