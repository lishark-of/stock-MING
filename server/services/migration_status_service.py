...
LONG_TERM_GOAL_PROGRESS = [
    ...
    },
    {
        ...
        "current_state": "schema/version preflight, manifest writer/validator, DuckDB read API, filters, cursor pagination, dry-runs, physical activation receipt, physical stage-scope manifest, physical execution recipe, scope-bound physical execution request ticket, current-result atomic promotion / retention POST readback, and physical durable evidence recipe exist.",
...
def _build_usable_path_current_checkpoint_rows(
    ...
    current_checkpoint_scope = {
        ...
        "P3": {
            ...
            "current_next_action": "普通页先看候选雷达、量化推演、次日图谱三段结果；浏览器 QA、provider-backed factor/radar evidence 另行验收。",
            "current_evidence_scope": "candidate_radar/quant_projection/next_session explainable readback is visible; research-only display is not production replacement evidence",
            "result_surfaces": "candidate_radar/quant_projection/next_session",
            "result_readback_source": "ordinary_result_quick_read_rows + ordinary_result_handoff_rows + next_session_ordinary_result_replay_rows",
            "result_evidence_boundary": "ordinary_result_rows_are_cache_only_not_provider_browser_or_production_replacement",
            "research_only_boundary": "candidate_not_buy_instruction_factor_scores_research_only_operation_zones_not_orders",
            "action_mutation_boundary": "does_not_trade_does_not_modify_strategy_action_or_operation_zones",
            "p3_next_gate": "provider_backed_factor_radar_evidence_plus_echarts_browser_performance_review",
        },
        "P4": {
        ...
        rows.append(
            {
                ...
                "result_surfaces": checkpoint_scope.get("result_surfaces", ""),
                "result_readback_source": checkpoint_scope.get("result_readback_source", ""),
                "result_evidence_boundary": checkpoint_scope.get("result_evidence_boundary", ""),
...
def _build_ltg_rebase_cycle_1_status(
    ...
    summary = {
        ...
        "next_goal_candidate": "rerun_local_push_gate_after_clean_worktree_for_current_head",
        "recommended_first_ltg_after_rebase": "LTG-11 clean local checkpoint / release-boundary review",
...
def _build_ltg_strict_closeout_work_order_rows(
    *,
    ...
    remote_receipt_missing_evidence = [
        str(item)
        for item in release_gate_direct_evidence.get("remote_ci_review_receipt_missing_evidence") or []
        if str(item)
    ]
    if release_gate_direct_evidence.get("remote_ci_review_receipt_stale_for_current_head") is True:
        ...
        rows.append(
            {
                ...
                "release_gate_remote_ci_review_receipt_blocks_current_head_remote_review": (
                    release_gate_direct_evidence.get(
    ...
    summary = {
        ...
        "release_gate_remote_ci_review_receipt_blocks_current_head_remote_review": (
            release_gate_direct_evidence.get("remote_ci_review_receipt_blocks_current_head_remote_review")
            is True
        ),
        "release_gate_local_git_status_is_not_ci_status": True,
        "external_calls_triggered": False,
...
def _build_usable_path_medium_goal_checkpoint(
    *,
    ...
    summary = {
        ...
        "active_remaining_goal_mode": "remaining_ltg_ready_button_first_closeout_queue_with_remote_review_split",
        "completed_medium_goals_removed_from_active_plan": True,
        "next_recommended_action": "rerun_local_push_gate_after_clean_worktree_for_current_head",
        "recommended_first_ltg_after_medium_goals": ["LTG-11"],
        "support_ltg_for_next_slice": ["LTG-11"],
        "remote_review_split_required_before_remote_claim": True,
...
def _latest_factor_test_lab_production_validation_handoff_summary() -> dict[str, Any]:
    try:
        from server.services import factor_service
    ...
    packet_map = packet if isinstance(packet, dict) else {}
    factor_tests = _dict_or_empty(packet_map.get("factor_tests"))
    production_qa = _dict_or_empty(factor_tests.get("production_validation_qa_contract"))
    provider_blocker = _dict_or_empty(factor_tests.get("provider_validation_blocker_audit"))
    durable_recipe = _dict_or_empty(factor_tests.get("durable_evidence_recipe"))
    durable_rows = [
        row for row in factor_tests.get("durable_evidence_rows") or [] if isinstance(row, dict)
    ]
    provider_handoff = _latest_factor_test_lab_provider_validation_handoff_summary()
    execution_request_ready = (
    ...
    scope_or_recipe_ready = (
        ...
        or provider_handoff.get("provider_small_pool_scope_ticket_verified") is True
    )
    production_qa_ready = (
        production_qa.get("status") == "production_validation_qa_contract_ready_provider_execution_pending"
    )
    durable_recipe_ready = durable_recipe.get("local_recipe_ready") is True
    ...
    missing_evidence_items = [
        ...
        "neutralization stability validation",
        "point-in-time bias controls",
        "manual Factor Test production promotion review",
        "full-market validation boundary review",
    ...
    if provider_sample_done:
        ...
        status = "factor_test_production_handoff_scope_ready_execution_request_pending"
        next_step = "POST /api/factor-quant/provider-small-pool-execution-request"
    else:
        status = "factor_test_production_handoff_scope_ticket_needed"
        next_step = "POST /api/factor-quant/provider-small-pool-dry-run"
    return {
        ...
        "provider_validation_handoff_status": provider_handoff.get("status") or "missing",
        "production_validation_qa_status": production_qa.get("status") or "missing",
        "production_validation_qa_ready": production_qa_ready,
        "provider_validation_blocker_status": provider_blocker.get("status") or "missing",
        "provider_validation_blocker_count": int(
            provider_blocker.get("production_blocker_count") or 0
        ),
        "local_light_metric_baseline_verified": (
            provider_handoff.get("local_light_metric_baseline_verified") is True
        ),
        "provider_small_pool_scope_ticket_verified": (
            provider_handoff.get("provider_small_pool_scope_ticket_verified") is True
        ),
        "provider_small_pool_dry_run_ready": (
            provider_handoff.get("provider_small_pool_dry_run_ready") is True
        ),
        "provider_small_pool_execution_recipe_ready": (
            provider_handoff.get("provider_small_pool_execution_recipe_ready") is True
        ),
        "provider_small_pool_execution_request_ready": execution_request_ready,
        "provider_small_pool_scope_hash_short": provider_handoff.get(
            "provider_small_pool_scope_hash_short", ""
        ),
        "target_provider_task_route": provider_handoff.get("target_provider_task_route", ""),
        "target_provider_task_type": provider_handoff.get("target_provider_task_type", ""),
        "target_acceptance_mode": provider_handoff.get("target_acceptance_mode", ""),
        "symbol_count": int(provider_handoff.get("symbol_count") or 0),
        "metric_count": int(provider_handoff.get("metric_count") or 0),
        "durable_recipe_status": durable_recipe.get("status") or "missing",
        "durable_recipe_ready": durable_recipe_ready,
        "durable_evidence_row_count": len(durable_rows),
        ...
        "next_local_step": next_step,
        "evidence_boundary": (
            "factor_test_production_handoff_is_local_durable_recipe_readback_not_provider_execution_or_ltg_closeout"
        ),
    }
...
def _latest_release_gate_direct_evidence_summary() -> dict[str, Any]:
    ...
    remote_receipt_present = str(remote_receipt_map.get("read_status") or "") == "receipt_present"
    ...
    return {
        ...
        "remote_ci_review_receipt_stale_for_current_head": remote_receipt_stale_for_current_head,
        "remote_ci_review_receipt_missing_evidence": remote_receipt_missing_evidence,
        "remote_ci_review_receipt_missing_evidence_count": len(remote_receipt_missing_evidence),
    ...
    return [
        ...
        },
        {
            ...
            "remote_ci_review_receipt_stale_for_current_head": (
                release_gate_summary.get("remote_ci_review_receipt_stale_for_current_head") is True
            ),
            "remote_ci_review_receipt_missing_evidence": list(
                release_gate_summary.get("remote_ci_review_receipt_missing_evidence") or []
            ),
            "remote_ci_review_receipt_missing_evidence_count": int(
                release_gate_summary.get("remote_ci_review_receipt_missing_evidence_count") or 0
            ),
            "remote_ci_review_receipt_blocks_current_head_remote_review": (
                release_gate_summary.get("remote_ci_review_receipt_blocks_current_head_remote_review")
                is True
            ),
            "github_api_called": False,
            "external_calls_triggered": False,
...
def _latest_release_gate_remote_review_handoff_summary() -> dict[str, Any]:
    ...
    return {
        ...
        "remote_ci_review_receipt_head_matches_current": remote_receipt_head_matches_current,
        "remote_ci_review_receipt_stale_for_current_head": (
            release_gate.get("remote_ci_review_receipt_stale_for_current_head") is True
        ),
        "remote_ci_review_receipt_missing_evidence": remote_receipt_missing_evidence,
        "remote_ci_review_receipt_missing_evidence_count": len(remote_receipt_missing_evidence),
        "remote_ci_review_receipt_blocks_current_head_remote_review": (
...
def _latest_motion_production_handoff_summary() -> dict[str, Any]:
    try:
        ...
        motion_contract = {}
    motion_contract_map = motion_contract if isinstance(motion_contract, dict) else {}
    ...
    packet_map = audit_packet if isinstance(audit_packet, dict) else {}
    evidence = _dict_or_empty(packet_map.get("motion_browser_qa_evidence_contract"))
    review = _dict_or_empty(packet_map.get("motion_browser_qa_review_contract"))
    activation = _dict_or_empty(packet_map.get("motion_production_activation_receipt"))
    dry_run = _dict_or_empty(packet_map.get("motion_promotion_dry_run_receipt"))
    promotion_review = _dict_or_empty(
        packet_map.get("motion_visual_performance_promotion_review_receipt")
    )
    durable_recipe = _dict_or_empty(packet_map.get("motion_durable_evidence_recipe"))
    browser_visual_ready = evidence.get("visual_qa_complete") is True
    browser_performance_ready = evidence.get("browser_performance_verified") is True
    reduced_motion_ready = evidence.get("reduced_motion_passed") is True
    browser_qa_review_ready = review.get("local_browser_qa_review_ready") is True
    activation_ready = activation.get("local_activation_receipt_ready") is True
    dry_run_ready = dry_run.get("ready_for_local_promotion_review") is True
    visual_performance_review_ready = (
        promotion_review.get("local_visual_performance_promotion_review_ready") is True
    )
    durable_recipe_ready = durable_recipe.get("local_recipe_ready") is True
    browser_visual_promoted = promotion_review.get("browser_visual_qa_promoted") is True
    browser_performance_promoted = promotion_review.get("browser_performance_promoted") is True
    reduced_motion_promoted = (
        promotion_review.get("reduced_motion_durable_evidence_promoted") is True
    )
    local_release_gate_evidence_observed = bool(
        (
    ...
    direct_stage_count = len(direct_stage_keys)
    try:
        row_count = int(
            motion_contract_map.get("motion_production_stage_scope_count")
            or durable_recipe.get("row_count")
            or 0
        )
    except (TypeError, ValueError):
        row_count = 0
    pending_stage_count = max(row_count - direct_stage_count, 0) if row_count else 0
    production_blocker_count = int(
        promotion_review.get("production_blocker_count")
        or durable_recipe.get("production_blocker_count")
        or pending_stage_count
        or 0
    )
    durable_ci_evidence_complete = bool(
        promotion_review.get("durable_ci_evidence_complete") is True
        or durable_recipe.get("durable_ci_evidence_complete") is True
        or promotion_review.get("ci_evidence_complete") is True
        or durable_recipe.get("ci_evidence_complete") is True
    )
    if visual_performance_review_ready:
        status = "motion_visual_performance_promotion_review_ready_ci_release_blocked"
        next_local_step = "attach durable CI/release evidence and final motion production review before closeout"
    elif dry_run_ready:
        status = "motion_promotion_dry_run_ready_visual_performance_review_needed"
        next_local_step = "POST /api/audit/motion-visual-performance-promotion-review"
    elif browser_qa_review_ready:
        status = "motion_browser_qa_review_ready_promotion_dry_run_needed"
        next_local_step = "POST /api/audit/motion-production-promotion-dry-run"
    elif activation_ready:
        status = "motion_activation_receipt_ready_browser_qa_review_needed"
        next_local_step = "POST /api/audit/motion-browser-qa-review"
    else:
        status = "motion_activation_receipts_visible_browser_qa_pending"
        next_local_step = "POST /api/audit/motion-browser-qa-review"
    return {
        "schema_version": "ltg14_motion_production_handoff_summary.v1",
        "source_packet_key": "command_center_3_call_ledger_audit_cache",
        "status": status,
        "next_local_step": next_local_step,
        "direct_evidence_stage_keys": direct_stage_keys,
        ...
        "pending_stage_count": pending_stage_count,
        "production_blocker_count": production_blocker_count,
        "motion_production_activation_receipt_ready": activation_ready,
        "motion_production_activation_receipt_status": str(activation.get("status") or ""),
        "motion_browser_qa_review_ready": browser_qa_review_ready,
        "motion_browser_qa_review_status": str(review.get("status") or ""),
        "motion_promotion_dry_run_ready": dry_run_ready,
        "motion_promotion_dry_run_status": str(dry_run.get("status") or ""),
        "motion_visual_performance_promotion_review_ready": visual_performance_review_ready,
        "motion_visual_performance_promotion_review_status": str(
            promotion_review.get("status") or ""
        ),
        "motion_visual_performance_promotion_review_task_id": str(
            promotion_review.get("promotion_review_task_id") or ""
        ),
        "motion_durable_evidence_recipe_ready": durable_recipe_ready,
        "motion_durable_evidence_recipe_status": str(durable_recipe.get("status") or ""),
        ...
        "local_motion_review_chain_ready_for_release_evidence": visual_performance_review_ready,
        "requires_durable_ci_release_evidence": not durable_ci_evidence_complete,
        "requires_production_motion_review": True,
        "requires_remote_ci_review_after_local_complete": True,
        "requires_release_review_after_remote_green": True,
        "cache_get_creates_task": False,
        "cache_get_opens_browser": False,
        "cache_get_runs_motion_runner": False,
        "cache_get_writes_artifacts": False,
        "cache_get_calls_provider": False,
        "cache_get_calls_model": False,
        "cache_get_calls_github": False,
        "creates_task_from_get": False,
        "opens_browser_from_get": False,
        "runs_motion_runner_from_get": False,
        "writes_artifacts_from_get": False,
        "external_calls_triggered": False,
        "tushare_called": False,
        "deepseek_called": False,
        "github_called": False,
...
def _latest_tushare_target_sample_evidence_handoff_summary() -> dict[str, Any]:
    ...
    packet_map = _dict_or_empty(packet)
    counts = _dict_or_empty(packet_map.get("counts"))
    latest_request = _dict_or_empty(
        packet_map.get("latest_tushare_provider_target_sample_execution_request")
    )
    try:
        refresh_packet = packet_service.read_packet("command_center_tushare_refresh_packet")
    except Exception:
    ...
    execution_request_ready = (
        ...
        and latest_request.get("ready_for_manual_provider_task_submission") is True
        and latest_request.get("execution_recipe_scope_hash_matches_latest") is True
        and int(latest_request.get("blocking_row_count") or 0) == 0
    )
    ...
    requested_targets = [
        str(item)
        for item in (
            latest_request.get("requested_targets") or target_contract.get("requested_targets") or []
        )
        if str(item or "")
    ]
    ...
    return {
        ...
        "status": (
            ...
            if latest_request_found
            else "target_sample_execution_request_needed"
        ),
        "source_packet_key": "command_center_3_data_health_timeline_cache+command_center_tushare_refresh_packet",
        "latest_execution_request_found": latest_request_found,
        "latest_execution_request_status": latest_request.get("execution_request_status")
        or latest_request.get("status")
        or "missing",
        "latest_execution_request_task_id": str(latest_request.get("latest_task_id") or ""),
        "latest_execution_request_ready_for_manual_provider_task_submission": execution_request_ready,
        "latest_execution_request_scope_hash_matches": (
            latest_request.get("execution_recipe_scope_hash_matches_latest") is True
        ),
        "latest_execution_request_row_count": int(
            counts.get("latest_tushare_provider_target_sample_execution_request_row_count") or 0
        ),
        "latest_execution_request_blocking_row_count": int(
            counts.get("latest_tushare_provider_target_sample_execution_request_blocking_row_count")
            or latest_request.get("blocking_row_count")
            or 0
        ),
        "target_post_task_route": latest_request.get("target_post_task_route")
        or "POST /api/tasks/refresh-tushare-facts",
        "target_task_type": latest_request.get("target_task_type") or "refresh_tushare_facts",
        "target_acceptance_mode": latest_request.get("target_acceptance_mode")
        or "provider_target_sample_acceptance",
        "requested_targets": requested_targets,
        "requested_target_count": len(requested_targets),
        "selected_apis": selected_apis,
        "selected_api_count": len(selected_apis),
        "target_sample_acceptance_status": target_contract.get("status") or "missing",
        "target_sample_acceptance_ready_for_review": target_sample_review_ready,
        "target_sample_acceptance_ready_count": int(target_contract.get("ready_target_count") or 0),
...
def _build_ltg_next_acceptance_action_rows(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for index, action in enumerate(LTG_NEXT_ACCEPTANCE_ACTION_QUEUE, start=1):
        ...
        supporting_factor_test_lab_production_validation_handoff: dict[str, Any] = {}
        ...
        action_rows.append(
            {
                ...
                "supporting_tushare_target_sample_execution_request_ready": (
                    supporting_tushare_target_sample_evidence_handoff.get(
                        "latest_execution_request_ready_for_manual_provider_task_submission"
                    )
                    is True
                ),
                "supporting_tushare_target_sample_requires_provider_task": (
                    supporting_tushare_target_sample_evidence_handoff.get(
                        "requires_separate_user_approved_provider_task"
                    )
                    is True
                ...
                "supporting_factor_test_lab_production_validation_next_local_step": (
                    supporting_factor_test_lab_production_validation_handoff.get("next_local_step", "")
                ),
                "supporting_factor_test_lab_production_validation_qa_ready": (
                    supporting_factor_test_lab_production_validation_handoff.get(
                        "production_validation_qa_ready"
                    )
                    is True
                ),
                "supporting_factor_test_lab_production_validation_durable_ready": (
                    supporting_factor_test_lab_production_validation_handoff.get("durable_recipe_ready")
                    is True
                ),
                "supporting_factor_test_lab_production_validation_durable_blocker_count": int(
                    supporting_factor_test_lab_production_validation_handoff.get(
                        "durable_evidence_blocker_count"
                    )
                    or 0
                ),
                "supporting_factor_test_lab_production_validation_requires_provider_task": (
                    supporting_factor_test_lab_production_validation_handoff.get(
                        "requires_separate_user_approved_provider_task"
                    )
                    is True
                ...
                "supporting_factor_test_lab_production_validation_creates_task_from_get": (
                    supporting_factor_test_lab_production_validation_handoff.get("cache_get_creates_task")
                    is True
                ),
                "supporting_candidate_radar_production_handoff": (
                    supporting_candidate_radar_production_handoff
                ...
                "supporting_motion_production_handoff": supporting_motion_production_handoff,
                "supporting_motion_production_next_local_step": (
                    supporting_motion_production_handoff.get("next_local_step", "")
                ),
                "supporting_motion_production_review_chain_ready_for_release_evidence": (
                    supporting_motion_production_handoff.get(
                        "local_motion_review_chain_ready_for_release_evidence"
                    )
                    is True
                ),
                "supporting_motion_production_requires_durable_ci_release_evidence": (
                    supporting_motion_production_handoff.get("requires_durable_ci_release_evidence")
                    is True
                ),
                "supporting_motion_production_requires_final_review": (
                    supporting_motion_production_handoff.get("requires_production_motion_review") is True
                ),
...
def _build_ltg_stage_scope_observed_rows() -> list[dict[str, Any]]:
    ...
    try:
        ...
        ltg04_local_complete = bool(row_count and direct_evidence_count >= row_count)
        ltg04_remote_review_pending = bool(ltg04_local_complete and not latest_remote_run_verified_green)
        ltg04_release_review_pending = bool(ltg04_local_complete and latest_remote_run_verified_green)
        ltg04_local_completion_status = (
            "local_factor_universe_direct_research_evidence_complete"
            if ltg04_local_complete
        ...
        rows.append(
            {
                ...
                "direct_evidence_stage_count": direct_evidence_count,
                "direct_evidence_stage_keys": direct_evidence.get("direct_evidence_stage_keys", []),
                "local_complete": ltg04_local_complete,
                "local_completion_status": ltg04_local_completion_status,
                "local_blocker_count": 0 if ltg04_local_complete else effective_pending_count,
                "remote_review_required_after_local_complete": True,
                "remote_review_pending": ltg04_remote_review_pending,
                "remote_review_status": (
                    "remote_review_pending"
                    if ltg04_remote_review_pending
                    else "remote_review_green_release_review_pending"
                    if ltg04_release_review_pending
                    else "remote_review_waiting_for_local_complete"
                ),
                "remote_review_pending_count": 1 if ltg04_remote_review_pending else 0,
                "release_review_required_after_remote_green": True,
                "release_review_pending": ltg04_release_review_pending,
                "release_review_status": (
                    "release_review_pending"
                    if ltg04_release_review_pending
                    else "release_review_waiting_for_remote_green"
                ...
                "release_review_pending_count": 1 if ltg04_release_review_pending else 0,
                "strict_closeout_ready": False,
                "missing_evidence_items": [
                    "factor universe direct research evidence for all stage rows",
                    "production worker-backed batch execution evidence",
                    "neutralization and full-pool validation evidence",
                    "matching remote CI review after local Factor universe evidence",
                    "release review after matching remote CI green",
                ],
    ...
    try:
        ...
        ltg05_local_complete = bool(row_count and direct_evidence_count >= row_count)
        ltg05_remote_review_pending = bool(ltg05_local_complete and not latest_remote_run_verified_green)
        ltg05_release_review_pending = bool(ltg05_local_complete and latest_remote_run_verified_green)
        ltg05_local_completion_status = (
            "local_storage_direct_evidence_complete"
            if ltg05_local_complete
            else "local_storage_direct_evidence_partial"
            if direct_evidence_count
            else "local_static_contract_only"
        )
        storage_status = (
        ...
        rows.append(
            {
                ...
                "local_complete": ltg05_local_complete,
                "local_completion_status": ltg05_local_completion_status,
                "local_blocker_count": 0 if ltg05_local_complete else observed_pending_count,
                "remote_review_required_after_local_complete": True,
                "remote_review_pending": ltg05_remote_review_pending,
                "remote_review_status": (
                    "remote_review_pending"
                    if ltg05_remote_review_pending
                    else "remote_review_green_release_review_pending"
                    if ltg05_release_review_pending
                    else "remote_review_waiting_for_local_complete"
                ),
                "remote_review_pending_count": 1 if ltg05_remote_review_pending else 0,
                "release_review_required_after_remote_green": True,
                "release_review_pending": ltg05_release_review_pending,
                "release_review_status": (
                    "release_review_pending"
                    if ltg05_release_review_pending
                    else "release_review_waiting_for_remote_green"
                ...
                "release_review_pending_count": 1 if ltg05_release_review_pending else 0,
                "strict_closeout_ready": False,
                "missing_evidence_items": [
                    "physical storage direct evidence for all stage rows",
                    "production storage promotion review",
                    "matching remote CI review after local storage evidence",
                    "release review after matching remote CI green",
                ],
                "production_blocker_count": observed_pending_count,
    ...
    try:
        ...
        ltg07_local_complete = bool(row_count and direct_evidence_count >= row_count)
        ltg07_remote_review_pending = bool(ltg07_local_complete and not latest_remote_run_verified_green)
        ltg07_release_review_pending = bool(ltg07_local_complete and latest_remote_run_verified_green)
        ltg07_local_completion_status = (
            "local_deepseek_direct_evidence_complete"
            if ltg07_local_complete
            else "local_deepseek_direct_evidence_partial"
            if direct_evidence_count
            else "local_static_contract_only"
        )
        rows.append(
            {
                ...
                "remote_review_required_after_local_complete": True,
                "remote_review_pending": ltg07_remote_review_pending,
                "remote_review_status": (
                    "remote_review_pending"
                    if ltg07_remote_review_pending
                    else "remote_review_green_release_review_pending"
                    if ltg07_release_review_pending
                    else "remote_review_waiting_for_local_complete"
                ),
                "remote_review_pending_count": 1 if ltg07_remote_review_pending else 0,
                "release_review_required_after_remote_green": True,
                "release_review_pending": ltg07_release_review_pending,
                "release_review_status": (
                    "release_review_pending"
                    if ltg07_release_review_pending
                    else "release_review_waiting_for_remote_green"
                ...
                "release_review_pending_count": 1 if ltg07_release_review_pending else 0,
                "strict_closeout_ready": False,
                "missing_evidence_items": [
                    "provider-backed DeepSeek benchmark report",
                    "provider response_format/json_schema enforcement evidence",
                    "bounded retry/repair provider execution evidence",
                    "model ledger hash/dedupe and redaction evidence",
                    "token/cost budget evidence",
                    "auto-after-task mode gate evidence",
                    "production DeepSeek promotion review",
                    "matching remote CI review after local DeepSeek evidence",
                    "release review after matching remote CI green",
                ],
    ...
    try:
        ...
        ltg09_local_complete = bool(
            row_count
            and direct_evidence_count >= row_count
            and effective_production_blocker_count == 0
        )
        ltg09_remote_review_pending = bool(ltg09_local_complete and not latest_remote_run_verified_green)
        ltg09_release_review_pending = bool(ltg09_local_complete and latest_remote_run_verified_green)
        ltg09_local_completion_status = (
            "local_tauri_package_direct_evidence_complete"
            if ltg09_local_complete
            else "local_tauri_package_direct_evidence_blocked"
            if direct_evidence_count >= row_count and direct_evidence_count
            else "local_tauri_package_direct_evidence_partial"
            if direct_evidence_count
            else "local_static_contract_only"
        )
        tauri_status = (
        ...
        rows.append(
            {
                ...
                "local_complete": ltg09_local_complete,
                "local_completion_status": ltg09_local_completion_status,
                "local_blocker_count": 0 if ltg09_local_complete else effective_production_blocker_count,
                "remote_review_required_after_local_complete": True,
                "remote_review_pending": ltg09_remote_review_pending,
                "remote_review_status": (
                    "remote_review_pending"
                    if ltg09_remote_review_pending
                    else "remote_review_green_release_review_pending"
                    if ltg09_release_review_pending
                    else "remote_review_waiting_for_local_complete"
                ),
                "remote_review_pending_count": 1 if ltg09_remote_review_pending else 0,
                "release_review_required_after_remote_green": True,
                "release_review_pending": ltg09_release_review_pending,
                "release_review_status": (
                    "release_review_pending"
                    if ltg09_release_review_pending
                    else "release_review_waiting_for_remote_green"
                ...
                "release_review_pending_count": 1 if ltg09_release_review_pending else 0,
                "strict_closeout_ready": False,
                "missing_evidence_items": [
                    "macOS signing/notarization or explicit distribution waiver",
                    "production package promotion review",
                    "durable packaged-runtime release evidence",
                    "matching remote CI review after local Tauri package evidence",
                    "release review after matching remote CI green",
                ],
        ...
        rows.append(
            {
                ...
                "remote_review_status": (
                    "remote_review_pending"
                    if ltg12_remote_review_pending
                    else "remote_review_green_release_review_pending"
                    if ltg12_release_review_pending
                    else "remote_review_waiting_for_local_complete"
                ),
                "remote_review_pending_count": 1 if ltg12_remote_review_pending else 0,
                "release_review_required_after_remote_green": True,
                "release_review_pending": ltg12_release_review_pending,
                "release_review_status": (
                    "release_review_pending"
                    if ltg12_release_review_pending
                    else "release_review_waiting_for_remote_green"
                ...
                "release_review_pending_count": 1 if ltg12_release_review_pending else 0,
                "strict_closeout_ready": False,
                "missing_evidence_items": [
...
def build_migration_status() -> dict[str, Any]:
    ...
    ltg14_motion_production_handoff_summary = _latest_motion_production_handoff_summary()
        ...
        "call_ledger": [
            {
                ...
                "ltg06_worker_runtime_qa_handoff_status": (
                    ltg06_worker_runtime_qa_handoff_summary.get("status")
                ),
                ...
                "ltg14_motion_production_handoff_status": (
                    ltg14_motion_production_handoff_summary.get("status")
                ),
                "ltg14_motion_production_next_local_step": (
                    ltg14_motion_production_handoff_summary.get("next_local_step")
                ),
                "ltg13_candidate_radar_production_handoff_status": (
                    ltg13_candidate_radar_production_handoff_summary.get("status")
                ),
                "ltg13_candidate_radar_production_next_local_step": (
                    ltg13_candidate_radar_production_handoff_summary.get("next_local_step")
                ),
