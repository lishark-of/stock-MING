...
LONG_TERM_GOAL_PROGRESS = [
    {
        ...
        "current_state": "freshness gate MVP, local matrix, synthetic long-window replay, local trade_cal artifact audit, provider-acceptance runbook, dry-run scope ticket path, next execution recipe, and durable evidence recipe exist.",
    ...
    },
    {
        ...
        "current_state": "daily / daily_basic / moneyflow light path has real evidence; extended interfaces have matrix, local QA, runbook, dry-run contracts, target-sample execution recipe, production stage-scope manifest, and durable evidence recipe.",
    ...
    },
    {
        ...
        "current_state": "watchlist/custom/full-pool contracts, local read-plan receipts, readiness/activation receipts, local rank/zscore sufficiency audit, worker-batch dry-run scope ticket, and worker-batch execution recipe exist.",
    ...
    },
    {
        ...
        "completion_bucket": "productionization_required",
        "completion_estimate": "35%-45%",
        "current_state": "local fallback, task lifecycle, explicit synthetic healthcheck, button-gated activation review task receipts, production evidence plan scope tickets, runtime QA execution recipe, durable evidence recipe, readiness/activation receipts, scheduler default-off policy, and worker contracts exist.",
        "not_complete_because": "real Celery/Redis process orchestration, broker healthcheck, runtime queue binding, synthetic round-trip dispatch, cross-process control proof, append-only worker log proof, fallback rollback proof, durable evidence promotion, and production scheduler activation are pending.",
        "next_step": "Run the runtime QA execution recipe one phase at a time after manual approval, then promote only with durable evidence rows reviewed; keep Celery/Redis start, broker ping, task dispatch, provider/model calls, and scheduler enablement out of cache reads.",
        "production_complete": False,
    },
    {
        "id": "LTG-07",
