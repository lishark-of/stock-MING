...
DEEPSEEK_DURABLE_EVIDENCE_LABELS = {
    "manual_default_off_governance_visible": "Manual/default-off governance visible",
    "sanitizer_whitelist_visible": "Sanitizer whitelist visible",
    "json_stability_audit_visible": "JSON stability audit visible",
    "response_format_review_visible": "Response-format review visible",
    "retry_repair_dry_run_visible": "Retry/repair dry-run visible",
    "production_activation_receipt_visible": "Production activation receipt visible",
    "provider_benchmark_execution_recipe_visible": "Provider benchmark execution recipe visible",
    "provider_benchmark_report_required": "Provider benchmark report required",
    "provider_response_format_execution_required": "Provider response-format execution required",
    "bounded_retry_repair_execution_required": "Bounded retry/repair execution required",
    "model_ledger_hash_dedupe_required": "Model ledger/hash/dedupe evidence required",
    "sanitizer_parse_failed_provider_review_required": "Sanitizer and parse-failed provider review required",
    "token_budget_cost_evidence_required": "Token budget and cost evidence required",
    "auto_after_task_mode_gate_required": "auto_after_task mode gate required",
    "redaction_review_required": "Redaction review required",
...
FACTOR_UNIVERSE_DURABLE_EVIDENCE_LABELS = {
    ...
    "worker_batch_execution_request_visible": "Worker batch execution request visible",
...
FACTOR_TEST_DURABLE_EVIDENCE_SCHEMA_VERSION = "factor_test_durable_evidence_recipe.v1"
FACTOR_TEST_DURABLE_EVIDENCE_KEYS = (
...
FACTOR_TEST_DURABLE_EVIDENCE_LABELS = {
    "local_light_metric_baseline_visible": "Local light metric baseline visible",
    "storage_query_boundary_visible": "Storage query boundary visible",
    "local_dataset_sample_evidence_visible": "Local dataset sample evidence visible",
    "production_validation_qa_visible": "Production validation QA visible",
    "provider_blocker_audit_visible": "Provider blocker audit visible",
    "provider_sample_readiness_receipt_visible": "Provider sample readiness receipt visible",
    "provider_sample_activation_receipt_visible": "Provider sample activation receipt visible",
    "provider_small_pool_scope_ticket_visible": "Provider small-pool scope ticket visible",
    "provider_small_pool_execution_recipe_visible": "Provider small-pool execution recipe visible",
    "provider_small_pool_execution_request_visible": "Provider small-pool execution request visible",
    "provider_backed_small_pool_task_required": "Provider-backed small-pool task required",
    "safe_provider_call_ledger_required": "Safe provider call ledger required",
    "sample_row_collection_required": "Provider sample rows required",
    "multi_horizon_forward_returns_required": "Multi-horizon forward returns required",
    "rolling_ic_icir_required": "Rolling IC / Rank IC / ICIR required",
    "cost_turnover_validation_required": "Cost and turnover validation required",
    "neutralization_stability_required": "Neutralization stability required",
    "pit_bias_controls_required": "PIT/lookahead/survivorship controls required",
    "full_market_boundary_review_required": "Full-market boundary review required",
    "promotion_review_required": "Promotion review required",
...
def read_factor_quant_cache() -> dict[str, Any]:
    ...
    universe_batch_request_warning = "Factor Universe worker-batch execution request ticket 只绑定 dry-run scope hash 和后续手工 worker task 请求；不会创建任务、启动 worker、计算 rank/zscore/neutralization 或标记生产完成。"
    ...
    owned_warnings = {
        ...
        universe_batch_request_warning,
...
def _read_candidate_radar_quant_projection_handoff(now: str) -> dict[str, Any]:
    ...
    p3_ready = interpretation.get("interpretation_ready") is True or bool(str(p3_summary_raw).strip())
    ...
    p3_summary = _source_task_readback_text(
        p3_summary_raw,
...
def _factor_quant_candidate_handoff_rows(
    ...
    return [
        ...
        },
        {
            ...
            "当前状态": (
                f"源任务 Tushare ledger {handoff.get('provider_api_success_count')}/{handoff.get('provider_api_call_count')} 已回放；"
                "本次 GET Factor cache 未外联"
            ),
            ...
            "creates_task_from_readback": False,
            "external_calls_triggered": False,
            "source_task_tushare_called": handoff.get("source_task_tushare_called") is True,
            "readback_tushare_called": False,
            "contains_secret": False,
            "does_not_execute_trades": True,
            "does_not_modify_strategy_action": True,
...
def _factor_quant_p3_one_screen_summary(
    ...
    source_task_tushare_called = safe_handoff.get("source_task_tushare_called") is True
    source_task_tushare_provider_ledger_ready = safe_handoff.get("source_task_tushare_provider_ledger_ready") is True
    support_count = len(_list(score.get("support_factors")))
    suppress_count = len(_list(score.get("suppress_factors")))
    ...
    summary = {
        ...
        "provider_api_call_count": provider_call_count,
        "provider_call_ledger_replayed_from_source_task": source_task_tushare_provider_ledger_ready,
        "source_task_external_calls_triggered": safe_handoff.get("source_task_external_calls_triggered") is True,
        "source_task_tushare_called": source_task_tushare_called,
        "source_task_tushare_provider_ledger_ready": source_task_tushare_provider_ledger_ready,
        "readback_external_calls_triggered": False,
        "readback_tushare_called": False,
        "ordinary_readback_provenance_summary": safe_handoff.get("ordinary_readback_provenance_summary") or "",
    ...
    items = [
        ...
        },
        {
            "label": "Tushare-first",
            "value": (
                f"源任务 ledger 已回放 {provider_success_count}/{provider_call_count} 个接口；本次 GET 未外联"
...
def _deepseek_durable_evidence_recipe_row(
    evidence_key: str,
    *,
    passed: bool,
    source_contract: str,
    ...
    return {
        ...
        "evidence_label": DEEPSEEK_DURABLE_EVIDENCE_LABELS.get(evidence_key, evidence_key),
...
def _deepseek_durable_evidence_recipe(
    hub: dict[str, Any],
    now: str,
) -> tuple[dict[str, Any], list[dict[str, Any]], list[dict[str, Any]]]:
    governance = hub.get("deepseek_explain_governance") if isinstance(hub.get("deepseek_explain_governance"), dict) else {}
    ...
    retry_repair = (
        hub.get("deepseek_retry_repair_dry_run_contract")
        if isinstance(hub.get("deepseek_retry_repair_dry_run_contract"), dict)
        else {}
    )
    activation_receipt = (
        hub.get("deepseek_production_activation_receipt")
    ...
    benchmark_recipe = (
        hub.get("deepseek_provider_benchmark_execution_recipe")
        if isinstance(hub.get("deepseek_provider_benchmark_execution_recipe"), dict)
        else {}
    )
    governance_visible = (
    ...
    json_visible = json_audit.get("schema_version") == "factor_deepseek_json_stability_audit.v1"
    response_visible = response_review.get("schema_version") == "factor_deepseek_response_format_review_contract.v1"
    retry_visible = retry_repair.get("schema_version") == "factor_deepseek_retry_repair_dry_run_contract.v1"
    activation_visible = activation_receipt.get("schema_version") == "deepseek_production_activation_receipt.v1"
    benchmark_visible = (
    ...
    no_model_trade_action_secret_boundary = (
        benchmark_recipe.get("provider_model_called_by_recipe") is False
        and benchmark_recipe.get("cache_get_external_calls") is False
        and benchmark_recipe.get("external_calls_triggered") is False
        and benchmark_recipe.get("tushare_called") is False
        and benchmark_recipe.get("deepseek_called") is False
        and benchmark_recipe.get("github_called") is False
        and benchmark_recipe.get("contains_secret") is False
    ...
    rows = [
        _deepseek_durable_evidence_recipe_row(
            "manual_default_off_governance_visible",
            passed=governance_visible,
            source_contract="deepseek_explain_governance",
            evidence=f"mode={governance.get('mode')}; auto_after_task={governance.get('auto_after_task')}; configured_auto={governance.get('configured_auto_after_task')}",
            required_evidence="manual/default-off governance visible with cache/render no-model-call boundaries",
            next_action="keep auto_after_task disabled until provider benchmark, response-format, retry/repair, cost, and promotion pass",
        ),
        _deepseek_durable_evidence_recipe_row(
            "sanitizer_whitelist_visible",
            passed=sanitizer_visible,
            source_contract="deepseek_validation_summary",
            evidence=f"validation_mode={validation.get('validation_mode')}; model_call_status={validation.get('model_call_status')}",
            required_evidence="local six-field sanitizer visible and numeric/action overwrite blocked",
            next_action="reuse sanitizer for provider benchmark review without treating it as provider evidence",
        ),
        _deepseek_durable_evidence_recipe_row(
            "json_stability_audit_visible",
            passed=json_visible,
            source_contract="deepseek_json_stability_audit",
            evidence=f"status={json_audit.get('status')}; last_rate={json_audit.get('last_known_mini_benchmark_success_rate')}; target={json_audit.get('required_json_success_rate')}",
            required_evidence="local JSON stability audit visible with production blockers explicit",
            next_action="replace local mini-benchmark signal with direct provider benchmark evidence",
        ),
        _deepseek_durable_evidence_recipe_row(
            "response_format_review_visible",
            passed=response_visible,
        ...
        _deepseek_durable_evidence_recipe_row(
            "retry_repair_dry_run_visible",
            passed=retry_visible,
            source_contract="deepseek_retry_repair_dry_run_contract",
            evidence=f"status={retry_repair.get('status')}; case_count={retry_repair.get('case_count')}",
            required_evidence="local retry/repair dry-run visible without model calls",
            next_action="run bounded retry/repair only inside a later approved provider benchmark task",
        ),
        _deepseek_durable_evidence_recipe_row(
            "production_activation_receipt_visible",
            passed=activation_visible,
            source_contract="deepseek_production_activation_receipt",
            evidence=f"status={activation_receipt.get('status')}; blockers={activation_receipt.get('blocking_criterion_count')}",
            required_evidence="production activation receipt visible and still provider-benchmark pending",
            next_action="preserve activation receipt as a gate, not production completion",
        ),
        _deepseek_durable_evidence_recipe_row(
            "provider_benchmark_execution_recipe_visible",
            passed=benchmark_visible,
            source_contract="deepseek_provider_benchmark_execution_recipe",
            evidence=f"status={benchmark_recipe.get('status')}; required_sample_count={benchmark_recipe.get('required_sample_count')}",
            required_evidence="provider benchmark execution recipe visible with no model call",
            next_action="bind any future model execution to the recipe and user approval",
        ),
        _deepseek_durable_evidence_recipe_row(
            "provider_benchmark_report_required",
            passed=False,
            source_contract="explicit_provider_benchmark_task",
            evidence="No provider-backed benchmark report with >=40 samples and >90% JSON success rate has been collected.",
            required_evidence="provider benchmark report with sample rows, JSON success rate, parse status, and safe call ledger",
            next_action="run the explicit benchmark task only after user approval and server-side credential presence check",
        ),
        _deepseek_durable_evidence_recipe_row(
            "provider_response_format_execution_required",
            passed=False,
        ...
        _deepseek_durable_evidence_recipe_row(
            ...
            passed=False,
            source_contract="explicit_provider_benchmark_task",
            evidence="Bounded retry/repair has only local dry-run coverage and has not run against provider responses.",
            required_evidence="bounded retry/repair ledger with per-sample retry counts and discard reasons",
            next_action="prove retry/repair under the benchmark retry budget before any auto_after_task promotion",
        ),
        _deepseek_durable_evidence_recipe_row(
            "model_ledger_hash_dedupe_required",
            passed=False,
            source_contract="explicit_provider_benchmark_task",
            evidence="No provider model ledger with input/output hashes, token usage, and cache hit/miss rows has been collected.",
            required_evidence="redacted model ledger with model_used, status, token_usage, parse_status, input_hash, output_hash, and cache_hit_or_miss",
            next_action="write only redacted ledger fields and never raw prompt or token/key material",
        ),
        _deepseek_durable_evidence_recipe_row(
            "sanitizer_parse_failed_provider_review_required",
            passed=False,
            source_contract="explicit_provider_benchmark_task",
            evidence="Sanitizer and parse-failed discard behavior has not been reviewed on provider responses.",
            required_evidence="provider-response sanitizer review proving illegal fields ignored and parse_failed discarded",
            next_action="review every failed parse and illegal-field response before production promotion",
        ),
        _deepseek_durable_evidence_recipe_row(
            "token_budget_cost_evidence_required",
            passed=False,
            source_contract="explicit_provider_benchmark_task",
            evidence="Token and cost evidence is not durable across a provider benchmark sample set.",
            required_evidence="token totals, per-sample average, retry overhead, and cost estimate",
            next_action="record token/cost evidence before enabling any background explanation mode",
        ),
        _deepseek_durable_evidence_recipe_row(
            "auto_after_task_mode_gate_required",
            passed=False,
            source_contract="manual_promotion_review",
            evidence="auto_after_task remains default-off and is not production-ready.",
            required_evidence="manual mode-gate approval after benchmark, response-format, retry/repair, cost, and redaction pass",
            next_action="keep auto_after_task disabled by default and allow only explicit opt-in after promotion",
        ),
        _deepseek_durable_evidence_recipe_row(
            "redaction_review_required",
            passed=False,
            source_contract="manual_promotion_review",
            evidence="No durable redaction review has been attached to provider benchmark ledger outputs.",
            required_evidence="review proving no token/key, raw prompt secret, raw credential, or unsafe packet field enters logs/cache/packet",
            next_action="run redaction review on benchmark artifacts before production promotion",
        ),
        _deepseek_durable_evidence_recipe_row(
            "production_promotion_review_required",
            passed=False,
            source_contract="manual_promotion_review",
            evidence="DeepSeek production explanation promotion has not been reviewed or approved.",
            required_evidence="manual production promotion review covering benchmark, response_format, retry/repair, cost, redaction, no-trade, no-action, and no-numeric-overwrite",
            next_action="hold production_deepseek_explanation_complete=false until every durable evidence row is reviewed",
        ),
        _deepseek_durable_evidence_recipe_row(
            "no_model_trade_action_secret_boundary",
            passed=no_model_trade_action_secret_boundary,
            source_contract="deepseek_provider_benchmark_execution_recipe",
            evidence="Durable evidence recipe is cache-only and calls no model/provider/GitHub, executes no trades, mutates no action, overrides no numeric values, and exposes no secret.",
            required_evidence="cache/render/model/trade/action/secret boundaries remain false in cache, rows, and push gate",
            next_action="preserve no-model/no-provider/no-trade/no-action/no-secret boundary while adding future provider evidence",
        ),
    ]
    blocked_rows = [row["evidence_key"] for row in rows if row.get("production_blocker")]
    recipe = {
        "schema_version": DEEPSEEK_DURABLE_EVIDENCE_SCHEMA_VERSION,
        "status": (
            "deepseek_durable_evidence_recipe_ready_production_pending"
            if local_recipe_ready
            else "deepseek_durable_evidence_recipe_blocked_local_contract"
        ),
        "scope": "local_deepseek_durable_evidence_recipe_no_model_call",
        "ltg": "LTG-07/LTG-11",
        ...
        "required_evidence": [
            ...
            "token budget and cost evidence",
            "redaction review",
            "auto_after_task manual mode gate",
            "production promotion review",
        ],
        "not_allowed_next_steps": [
            ...
            "call DeepSeek from React render",
            "local retry/repair dry-run as provider benchmark",
            "promote auto_after_task from durable recipe",
            "raw token/key in prompt, ledger, packet, cache, or log",
    ...
    ledger = [
        {
            "api": "local_deepseek_durable_evidence_recipe",
            "request_params_safe": {
                "status": recipe["status"],
                "row_count": len(rows),
                "production_blocker_count": len(blocked_rows),
            ...
            "data_date": None,
            "local_fetched_at": now,
            "call_status": recipe["status"],
            "error_message_safe": "",
            **_local_ledger_boundary(),
        }
    ]
    recipe["call_ledger"] = ledger
    return recipe, rows, ledger


def _attach_deepseek_durable_evidence_recipe(
    hub: dict[str, Any],
    now: str,
) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    recipe, rows, ledger = _deepseek_durable_evidence_recipe(hub, now)
    hub["deepseek_durable_evidence_recipe"] = recipe
    hub["deepseek_durable_evidence_rows"] = rows
    governance = hub.get("deepseek_explain_governance")
    if isinstance(governance, dict):
        governance["durable_evidence_recipe_status"] = recipe["status"]
        governance["durable_evidence_recipe_ready"] = recipe["local_recipe_ready"]
        governance["durable_evidence_blocker_count"] = recipe["durable_evidence_blocker_count"]
    return hub, ledger
...
def _factor_test_provider_small_pool_execution_request_row(
    criterion: str,
    status: str,
    passed: bool,
    evidence: str,
    ...
    *,
    ...
    return {
        ...
        "status": status,
        "passed": bool(passed),
        "required_for_manual_provider_task_submission": bool(required),
        "blocks_manual_provider_task_submission": bool(required and not passed),
        "evidence": evidence,
        "next_action": next_action,
        "provider_task_created": False,
        "provider_execution_implemented": False,
...
def _factor_test_provider_small_pool_execution_request_payload(
    payload: Any,
    factor_tests: dict[str, Any],
    now: str,
) -> dict[str, Any]:
    dry_run = _dict(factor_tests.get("provider_small_pool_acceptance_dry_run_receipt"))
    recipe = _dict(factor_tests.get("provider_small_pool_execution_recipe"))
    payload_dict = payload if isinstance(payload, dict) else {}
    requested_scope_hash = str(
    ...
    return {
        ...
        "requested_scope_hash": requested_scope_hash,
        "latest_dry_run_scope_hash": str(dry_run.get("acceptance_scope_hash") or ""),
        "latest_dry_run_scope_hash_short": str(dry_run.get("acceptance_scope_hash_short") or ""),
        "latest_dry_run_status": str(dry_run.get("status") or ""),
        "execution_recipe_status": str(recipe.get("status") or ""),
        "execution_recipe_ready": bool(recipe.get("local_recipe_ready")),
        "scope_ticket_ready": bool(recipe.get("scope_ticket_ready")),
        "symbols": [str(item) for item in dry_run.get("symbols", []) if item][:FACTOR_TEST_PROVIDER_SMALL_POOL_SYMBOL_LIMIT],
        "symbol_count": int(dry_run.get("symbol_count") or 0),
        "start_date": dry_run.get("start_date"),
        "end_date": dry_run.get("end_date"),
        "window_days": int(dry_run.get("window_days") or 0),
        "metrics": [str(item) for item in dry_run.get("metrics", []) if item],
        "forward_return_horizons": [str(item) for item in dry_run.get("forward_return_horizons", []) if item],
        "required_datasets": list(FACTOR_TEST_PROVIDER_SMALL_POOL_ALLOWED_DATASETS),
        "phase_keys": list(FACTOR_TEST_PROVIDER_SMALL_POOL_EXECUTION_PHASES),
...
def _factor_test_provider_small_pool_execution_request_receipt(
    payload_safe: dict[str, Any],
    now: str,
) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    latest_scope_hash = str(payload_safe.get("latest_dry_run_scope_hash") or "")
    requested_scope_hash = str(payload_safe.get("requested_scope_hash") or "")
    requested_hash_matches_latest = bool(latest_scope_hash and requested_scope_hash and requested_scope_hash == latest_scope_hash)
    dry_run_visible = bool(
        payload_safe.get("latest_dry_run_status")
        and latest_scope_hash
        and str(payload_safe.get("latest_dry_run_status")) != "provider_small_pool_dry_run_blocked_preflight"
    )
    user_confirmed = payload_safe.get("approved_by_user") is True
    recipe_ready = bool(payload_safe.get("execution_recipe_ready") and payload_safe.get("scope_ticket_ready"))
    target_route_ok = payload_safe.get("target_provider_task_route") == "future POST /api/factor-quant/provider-small-pool-acceptance"
    rows = [
        _factor_test_provider_small_pool_execution_request_row(
            "latest_dry_run_scope_ticket_visible",
            "passed_latest_scope_ticket_visible" if dry_run_visible else "blocked_missing_or_blocked_dry_run_scope_ticket",
            dry_run_visible,
            f"latest_dry_run_status={payload_safe.get('latest_dry_run_status')}; scope_hash_short={payload_safe.get('latest_dry_run_scope_hash_short')}",
            "Run the provider small-pool dry-run first and keep the approved scope hash visible.",
        ),
        _factor_test_provider_small_pool_execution_request_row(
            "scope_hash_bound_to_latest_dry_run",
            "passed_scope_hash_match" if requested_hash_matches_latest else "blocked_scope_hash_missing_or_mismatch",
            requested_hash_matches_latest,
            f"requested_scope_hash_short={requested_scope_hash[:16]}; latest_scope_hash_short={payload_safe.get('latest_dry_run_scope_hash_short')}",
            "Bind the manual provider task request to the latest approved dry-run scope hash.",
        ),
        _factor_test_provider_small_pool_execution_request_row(
            "explicit_user_confirmation",
            "passed_user_confirmed" if user_confirmed else "blocked_user_confirmation_required",
            user_confirmed,
            f"approved_by_user={user_confirmed}",
            "Require an explicit user confirmation before the future provider task is submitted.",
        ),
        _factor_test_provider_small_pool_execution_request_row(
            "execution_recipe_ready",
            "passed_execution_recipe_ready" if recipe_ready else "blocked_execution_recipe_not_ready",
            recipe_ready,
            f"execution_recipe_status={payload_safe.get('execution_recipe_status')}; scope_ticket_ready={payload_safe.get('scope_ticket_ready')}",
            "Keep the ordered execution recipe visible before any real provider-backed validation.",
        ),
        _factor_test_provider_small_pool_execution_request_row(
            "target_provider_task_route_declared",
            "passed_target_route_declared" if target_route_ok else "blocked_target_route_mismatch",
            target_route_ok,
            f"target_provider_task_route={payload_safe.get('target_provider_task_route')}",
            "Use the future explicit provider-small-pool validation route; never create it from GET cache.",
        ),
        _factor_test_provider_small_pool_execution_request_row(
            "provider_task_not_created_by_request",
            "passed_no_provider_task_created",
            True,
            "Execution request ticket records intent only and does not create or execute the provider task.",
            "Submit a separate provider-backed task only after the user explicitly approves that later step.",
            required=False,
        ),
        _factor_test_provider_small_pool_execution_request_row(
            ...
            "passed_no_external_or_trade_side_effects",
            True,
            "Request ticket does not call Tushare, DeepSeek, GitHub, execute trades, or mutate strategy action.",
            "Keep Factor Test Lab research-only through provider validation and promotion review.",
            required=False,
        ),
        _factor_test_provider_small_pool_execution_request_row(
            "secret_redaction_boundary",
            "passed_no_secret_exposure",
            True,
            "Request ticket includes symbols, dates, metrics, horizons, target route, and scope hash only.",
            "Keep token/key material out of frontend, logs, packets, cache, and task payloads.",
            required=False,
        ),
    ]
    blockers = [row["criterion"] for row in rows if row["blocks_manual_provider_task_submission"]]
    if not dry_run_visible:
        status = "factor_test_provider_small_pool_execution_request_blocked_missing_dry_run_scope_ticket"
        allowed_next_step = "run_factor_test_provider_small_pool_acceptance_dry_run"
    elif not requested_hash_matches_latest:
        status = "factor_test_provider_small_pool_execution_request_blocked_scope_hash_mismatch"
        allowed_next_step = "resubmit_execution_request_with_latest_scope_hash"
    elif not user_confirmed:
        status = "factor_test_provider_small_pool_execution_request_blocked_user_confirmation_required"
        allowed_next_step = "confirm_execution_request"
    elif not recipe_ready:
        status = "factor_test_provider_small_pool_execution_request_blocked_local_recipe"
        allowed_next_step = "repair_provider_small_pool_execution_recipe"
    else:
        status = "factor_test_provider_small_pool_execution_request_ready_manual_provider_task_pending"
        allowed_next_step = "manual_submit_future_provider_small_pool_acceptance_task_bound_to_scope_ticket"
    ready = status == "factor_test_provider_small_pool_execution_request_ready_manual_provider_task_pending"
    receipt = {
        "schema_version": "factor_test_provider_small_pool_execution_request.v1",
        "status": status,
        "scope": "local_factor_test_provider_small_pool_execution_request_no_provider_execution",
        "created_at": now,
        "ltg": "LTG-03/LTG-02/LTG-11",
        "local_execution_request_ready": ready,
        "ready_for_manual_provider_task_submission": ready,
        "allowed_next_step": allowed_next_step,
        "target_provider_task_route": payload_safe.get("target_provider_task_route"),
        "target_provider_task_type": payload_safe.get("target_provider_task_type"),
        "target_acceptance_mode": payload_safe.get("target_acceptance_mode"),
        "latest_dry_run_status": payload_safe.get("latest_dry_run_status"),
        "execution_recipe_status": payload_safe.get("execution_recipe_status"),
        "requested_scope_hash_matches_latest": requested_hash_matches_latest,
        "acceptance_scope_hash": latest_scope_hash if requested_hash_matches_latest else "",
        "acceptance_scope_hash_short": str(payload_safe.get("latest_dry_run_scope_hash_short") or ""),
        "requested_scope_hash_short": requested_scope_hash[:16],
        "symbols": payload_safe.get("symbols") or [],
        ...
        "blocking_criteria": blockers,
        "not_allowed_next_steps": [
            "treat_execution_request_as_provider_validation",
            "create provider task from GET cache",
            "call Tushare from execution request",
            "call DeepSeek from execution request",
            "call GitHub from execution request",
            "compute production IC from execution request",
            "mutate strategy action",
            "real trade execution",
            "leak token/key",
        ],
        "missing_evidence": [
            "separate provider-backed task implementation",
    ...
    ledger = [
        {
            ...
            "request_params_safe": {
                ...
                "target_provider_task_route": payload_safe.get("target_provider_task_route"),
                "symbol_count": payload_safe.get("symbol_count") or 0,
                "metric_count": len(payload_safe.get("metrics") or []),
...
def _missing_factor_test_provider_small_pool_execution_request(now: str) -> tuple[dict[str, Any], list[dict[str, Any]], list[dict[str, Any]]]:
    row = _factor_test_provider_small_pool_execution_request_row(
        "execution_request_task_missing",
        "no_execution_request_task_found",
        False,
        "No button-gated provider small-pool execution request ticket has been recorded yet.",
        "Run the dry-run scope ticket, then generate an execution request ticket before the future provider task.",
    )
    receipt = {
        "schema_version": "factor_test_provider_small_pool_execution_request.v1",
        ...
        "blocking_criteria": ["execution_request_task_missing"],
        "rows": [row],
        "row_count": 1,
    }
    ledger = [
...
def _attach_factor_test_provider_small_pool_execution_request(
    packet: dict[str, Any],
    now: str,
) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    factor_tests = packet.get("factor_tests") if isinstance(packet.get("factor_tests"), dict) else {}
    factor_tests = dict(factor_tests)
    receipt = factor_tests.get("provider_small_pool_execution_request_receipt")
    rows = factor_tests.get("provider_small_pool_execution_request_rows")
    if not isinstance(receipt, dict):
        receipt, rows, ledger = _missing_factor_test_provider_small_pool_execution_request(now)
    else:
        receipt = dict(receipt)
        rows = rows if isinstance(rows, list) else receipt.get("rows") if isinstance(receipt.get("rows"), list) else []
        receipt["source_packet_present"] = True
        receipt["cache_get_initializes_execution_request"] = False
        ledger = list(receipt.get("call_ledger") or [])
    factor_tests["provider_small_pool_execution_request_receipt"] = receipt
    factor_tests["provider_small_pool_execution_request_rows"] = rows
    existing_test_ledger = factor_tests.get("call_ledger") if isinstance(factor_tests.get("call_ledger"), list) else []
    factor_tests["call_ledger"] = list(existing_test_ledger) + list(ledger)
    acceptance = factor_tests.get("acceptance_contract") if isinstance(factor_tests.get("acceptance_contract"), dict) else {}
    if acceptance:
        acceptance = dict(acceptance)
        acceptance["provider_small_pool_execution_request_status"] = receipt.get("status")
        acceptance["provider_small_pool_execution_request_ready"] = bool(receipt.get("local_execution_request_ready"))
        acceptance["provider_small_pool_execution_request_is_not_provider_execution"] = True
...
def _factor_test_durable_evidence_recipe_row(
    evidence_key: str,
    source_contract: str,
    status: str,
    *,
    ...
    return {
        "evidence_key": evidence_key,
        "evidence_label": FACTOR_TEST_DURABLE_EVIDENCE_LABELS.get(evidence_key, evidence_key),
        "scope": "factor_test_durable_evidence_recipe",
        "source_contract": source_contract,
        "status": status,
        "passed": bool(passed),
...
def _factor_test_durable_evidence_recipe(
    factor_tests: dict[str, Any], now: str
) -> tuple[dict[str, Any], list[dict[str, Any]], list[dict[str, Any]]]:
    quality = _dict(factor_tests.get("quality_summary"))
    storage_query = _dict(factor_tests.get("storage_query_consumption"))
    local_dataset = _dict(factor_tests.get("local_dataset_sample_evidence"))
    production_qa = _dict(factor_tests.get("production_validation_qa_contract"))
    provider_blocker = _dict(factor_tests.get("provider_validation_blocker_audit"))
    readiness = _dict(factor_tests.get("provider_sample_readiness_receipt"))
    activation = _dict(factor_tests.get("provider_sample_activation_receipt"))
    dry_run = _dict(factor_tests.get("provider_small_pool_acceptance_dry_run_receipt"))
    execution_recipe = _dict(factor_tests.get("provider_small_pool_execution_recipe"))
    execution_request = _dict(factor_tests.get("provider_small_pool_execution_request_receipt"))
    items = factor_tests.get("items") if isinstance(factor_tests.get("items"), list) else []

    local_light_visible = bool(
        items
        or quality
        or factor_tests.get("status") in {"ready", "scaffold_ready", "research_ready"}
        or factor_tests.get("computed_item_count")
    )
    storage_visible = storage_query.get("schema_version") == "factor_test_storage_query_consumption.v1"
    local_dataset_visible = local_dataset.get("schema_version") == "factor_test_local_dataset_sample_evidence.v1"
    production_qa_visible = production_qa.get("schema_version") == "factor_test_production_validation_qa_contract.v1"
    blocker_visible = provider_blocker.get("schema_version") == "factor_test_provider_validation_blocker_audit.v1"
    readiness_visible = readiness.get("schema_version") == "factor_test_provider_sample_readiness_receipt.v1"
    activation_visible = activation.get("schema_version") == "factor_test_provider_sample_activation_receipt.v1"
    dry_run_visible = bool(
        dry_run.get("schema_version") == "factor_test_provider_small_pool_acceptance_dry_run.v1"
        and dry_run.get("local_dry_run_ready") is True
    )
    execution_recipe_visible = bool(
        execution_recipe.get("schema_version") == "factor_test_provider_small_pool_execution_recipe.v1"
        and execution_recipe.get("local_recipe_ready") is True
    )
    execution_request_visible = bool(
        execution_request.get("schema_version") == "factor_test_provider_small_pool_execution_request.v1"
        and execution_request.get("cache_get_external_calls") is False
        and execution_request.get("provider_task_created") is False
        and execution_request.get("provider_execution_implemented") is False
    )

    rows = [
        _factor_test_durable_evidence_recipe_row(
            "local_light_metric_baseline_visible",
            "quality_summary",
            "visible_research_only_baseline" if local_light_visible else "blocked_missing_local_light_metrics",
            passed=local_light_visible,
            production_blocker=False,
            local_surface_required=True,
            evidence="local light IC/Rank IC/ICIR/group/cost/drawdown metrics are visible as research-only baseline",
            next_action="Keep local light metrics outside strategy action while collecting provider-backed evidence.",
        ),
        _factor_test_durable_evidence_recipe_row(
            "storage_query_boundary_visible",
            "storage_query_consumption",
            "visible_cache_only_boundary" if storage_visible else "blocked_missing_storage_query_boundary",
            passed=storage_visible,
            production_blocker=False,
            local_surface_required=True,
            evidence="storage query contract is visible and does not compute production metrics",
            next_action="Keep DuckDB/query rows as lineage until provider-backed validation runs.",
        ),
        _factor_test_durable_evidence_recipe_row(
            "local_dataset_sample_evidence_visible",
            "local_dataset_sample_evidence",
            "visible_not_validation" if local_dataset_visible else "blocked_missing_local_dataset_sample_evidence",
            passed=local_dataset_visible,
            production_blocker=False,
            local_surface_required=True,
            evidence="local dataset sample sufficiency audit is visible but is not provider validation",
            next_action="Use local sample sufficiency only to prepare the future provider-backed small-pool run.",
        ),
        _factor_test_durable_evidence_recipe_row(
            "production_validation_qa_visible",
            "production_validation_qa_contract",
            "visible_pending_provider_execution" if production_qa_visible else "blocked_missing_production_validation_qa",
            passed=production_qa_visible,
            production_blocker=False,
            local_surface_required=True,
            evidence="production QA contract lists provider-backed and full-market gaps",
            next_action="Do not promote Factor Test Lab until the QA blockers are cleared by direct evidence.",
        ),
        _factor_test_durable_evidence_recipe_row(
            "provider_blocker_audit_visible",
            "provider_validation_blocker_audit",
            "visible_blockers" if blocker_visible else "blocked_missing_provider_blocker_audit",
            passed=blocker_visible,
            production_blocker=False,
            local_surface_required=True,
            evidence="provider blocker audit centralizes pending production validation evidence",
            next_action="Use blocker rows as the review checklist for the next provider-backed validation pass.",
        ),
        _factor_test_durable_evidence_recipe_row(
            "provider_sample_readiness_receipt_visible",
            "provider_sample_readiness_receipt",
            "visible_local_receipt" if readiness_visible else "blocked_missing_provider_sample_readiness_receipt",
            passed=readiness_visible,
            production_blocker=False,
            local_surface_required=True,
            evidence="provider small-pool readiness receipt is visible and local-only",
            next_action="Use readiness only to choose the next explicit task; do not treat it as provider evidence.",
        ),
        _factor_test_durable_evidence_recipe_row(
            "provider_sample_activation_receipt_visible",
            "provider_sample_activation_receipt",
            "visible_local_activation_receipt" if activation_visible else "blocked_missing_provider_sample_activation_receipt",
            passed=activation_visible,
            production_blocker=False,
            local_surface_required=True,
            evidence="activation receipt is visible as a local checklist before provider execution",
            next_action="Run future provider execution only through explicit task creation after approval.",
        ),
        _factor_test_durable_evidence_recipe_row(
            "provider_small_pool_scope_ticket_visible",
            "provider_small_pool_acceptance_dry_run_receipt",
            "visible_scope_ticket" if dry_run_visible else "blocked_missing_scope_ticket",
            passed=dry_run_visible,
            production_blocker=False,
            local_surface_required=True,
            evidence="provider small-pool dry-run scope ticket is visible with safe scope hash",
            next_action="Bind future provider execution to the approved scope hash.",
        ),
        _factor_test_durable_evidence_recipe_row(
            "provider_small_pool_execution_recipe_visible",
            "provider_small_pool_execution_recipe",
            "visible_execution_recipe" if execution_recipe_visible else "blocked_missing_execution_recipe",
            passed=execution_recipe_visible,
            production_blocker=False,
            local_surface_required=True,
            evidence="provider small-pool execution recipe is visible as an ordered acceptance path",
            next_action="Execute phases only through a future explicit provider-backed validation task.",
        ),
        _factor_test_durable_evidence_recipe_row(
            "provider_small_pool_execution_request_visible",
            "provider_small_pool_execution_request_receipt",
            "visible_execution_request_ticket" if execution_request_visible else "blocked_missing_execution_request_ticket",
            passed=execution_request_visible,
            production_blocker=False,
            local_surface_required=True,
            evidence="provider small-pool execution request ticket is visible as a local scope-bound request or missing placeholder",
            next_action="Generate the request ticket from an approved dry-run before submitting a future provider task.",
        ),
        _factor_test_durable_evidence_recipe_row(
            "provider_backed_small_pool_task_required",
            "future_provider_task",
            "pending_explicit_provider_task",
            passed=False,
            production_blocker=True,
            evidence="explicit provider-backed Factor Test small-pool task_id is still required",
            next_action="Create a future user-approved provider-backed task bound to the scope hash.",
        ),
        _factor_test_durable_evidence_recipe_row(
            "safe_provider_call_ledger_required",
            "future_provider_call_ledger",
            "pending_safe_provider_call_ledger",
            passed=False,
            production_blocker=True,
            evidence="safe provider call ledger rows for target pool are still required",
            next_action="Collect row_count/data_date/local_fetched_at/call_status/error_message_safe without credential values.",
        ),
        _factor_test_durable_evidence_recipe_row(
            "sample_row_collection_required",
            "future_provider_samples",
            "pending_non_empty_provider_samples",
            passed=False,
            production_blocker=True,
            evidence="non-empty provider-backed sample rows are still required",
            next_action="Verify provider sample rows before computing research-grade metrics.",
        ),
        _factor_test_durable_evidence_recipe_row(
            "multi_horizon_forward_returns_required",
            "future_forward_return_labels",
            "pending_multi_horizon_forward_returns",
            passed=False,
            production_blocker=True,
            evidence="1d/5d or configured multi-horizon forward-return labels are still required",
            next_action="Attach horizon labels and row counts to the provider-backed validation report.",
        ),
        _factor_test_durable_evidence_recipe_row(
            "rolling_ic_icir_required",
            "future_rolling_metric_validation",
            "pending_rolling_ic_rank_ic_icir",
            passed=False,
            production_blocker=True,
            evidence="rolling IC/Rank IC/ICIR windows are still required",
            next_action="Validate rolling windows separately from the local light metrics.",
        ),
        _factor_test_durable_evidence_recipe_row(
            "cost_turnover_validation_required",
            "future_cost_turnover_validation",
            "pending_cost_turnover_validation",
            passed=False,
            production_blocker=True,
            evidence="cost and turnover assumption validation is still required",
            next_action="Review pre/post-cost results and turnover assumptions before promotion.",
        ),
        _factor_test_durable_evidence_recipe_row(
            "neutralization_stability_required",
            "future_neutralization_validation",
            "pending_neutralization_stability",
            passed=False,
            production_blocker=True,
            evidence="industry and market-cap neutralization stability evidence is still required",
            next_action="Validate neutral IC stability across provider-backed windows.",
        ),
        _factor_test_durable_evidence_recipe_row(
            "pit_bias_controls_required",
            "future_bias_controls",
            "pending_pit_lookahead_survivorship_controls",
            passed=False,
            production_blocker=True,
            evidence="PIT, lookahead, and survivorship controls are still required",
            next_action="Block promotion until bias controls are reviewed against provider-backed samples.",
        ),
        _factor_test_durable_evidence_recipe_row(
            "full_market_boundary_review_required",
            "future_full_market_review",
            "pending_full_market_boundary_review",
            passed=False,
            production_blocker=True,
            evidence="full-market boundary review is still required before small-pool evidence can be generalized",
            next_action="Keep small-pool results from being described as full-market validation.",
        ),
        _factor_test_durable_evidence_recipe_row(
            "promotion_review_required",
            "future_promotion_review",
            "pending_manual_promotion_review",
            passed=False,
            production_blocker=True,
            evidence="manual Factor Test production promotion review is still required",
            next_action="Promote only after direct provider evidence, metric validation, and no-action boundaries are reviewed.",
        ),
        _factor_test_durable_evidence_recipe_row(
            "no_trade_action_secret_boundary",
            "release_boundary",
            ...
            production_blocker=False,
            local_surface_required=True,
            evidence="recipe keeps metrics research-only, does not mutate strategy action, and exposes no credential values",
            next_action="Keep this invariant in every future provider/model/worker task.",
        ),
    ]
    local_blockers = [
        row["evidence_key"]
        for row in rows
        if row.get("local_surface_required") is True and row.get("passed") is not True
    ]
    production_blockers = [
        row["evidence_key"]
        for row in rows
        if row.get("production_blocker") is True and row.get("passed") is not True
    ]
    recipe_ready = not local_blockers
    status = (
        "factor_test_durable_evidence_recipe_ready_production_pending"
        if recipe_ready
        else "factor_test_durable_evidence_recipe_blocked_local_contract"
    )
    ledger = [
        {
            "api": "local_factor_test_durable_evidence_recipe",
            "request_params_safe": {
                "scope": "local_factor_test_durable_evidence_recipe_no_provider_execution",
                "local_recipe_ready": recipe_ready,
                "production_blocker_count": len(production_blockers),
            ...
            "data_date": dry_run.get("end_date"),
            "local_fetched_at": now,
            "call_status": "local_recipe_ready_production_pending" if recipe_ready else "local_recipe_blocked_local_contract",
            "error_message_safe": "",
            **_local_ledger_boundary(),
        }
    ]
    recipe = {
        "schema_version": FACTOR_TEST_DURABLE_EVIDENCE_SCHEMA_VERSION,
        "status": status,
        "scope": "local_factor_test_durable_evidence_recipe_no_provider_execution",
        "created_at": now,
        "ltg": "LTG-03/LTG-11/LTG-12",
        ...
        "env_key_name_exposed": False,
        "credential_value_exposed": False,
        "evidence_keys": list(FACTOR_TEST_DURABLE_EVIDENCE_KEYS),
        "local_blockers": local_blockers,
        "local_blocker_count": len(local_blockers),
        "production_blockers": production_blockers,
        "production_blocker_count": len(production_blockers),
        "required_evidence": [
            "explicit provider task_id bound to small-pool scope hash",
            "safe provider call ledger rows for target pool",
            "non-empty provider-backed sample rows",
            "multi-horizon forward-return labels",
            "rolling IC/Rank IC/ICIR evidence",
            ...
            "industry and market-cap neutralization stability evidence",
            "PIT, lookahead, and survivorship controls evidence",
            "full-market boundary review",
            "manual Factor Test production promotion review",
        ],
        "not_allowed_next_steps": [
            "treat durable recipe as production completion",
            "treat durable recipe as provider-backed small-pool validation",
            "call Tushare from GET cache",
            "call Tushare from React render",
            "call DeepSeek from this recipe",
            "call GitHub from this recipe",
            "compute production IC/Rank IC/ICIR in React",
            "local light metrics as production validation",
            "local dataset sample as provider evidence",
            "mutate strategy action",
            "real trade execution",
            "leak token/key",
        ],
        "rows": rows,
        "call_ledger": ledger,
        "note": "This local LTG-03 durable evidence recipe fixes the direct evidence bundle required before Factor Test Lab production promotion. It does not call providers/models, compute production metrics, execute trades, mutate strategy action, expose secrets, or prove production completion.",
    }
    return recipe, rows, ledger


def _attach_factor_test_durable_evidence_recipe(
    packet: dict[str, Any], now: str
) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    factor_tests = packet.get("factor_tests") if isinstance(packet.get("factor_tests"), dict) else {}
    factor_tests = dict(factor_tests)
    recipe, rows, ledger = _factor_test_durable_evidence_recipe(factor_tests, now)
    factor_tests["durable_evidence_recipe"] = recipe
    factor_tests["durable_evidence_rows"] = rows
    existing_test_ledger = factor_tests.get("call_ledger") if isinstance(factor_tests.get("call_ledger"), list) else []
    factor_tests["call_ledger"] = list(existing_test_ledger) + ledger
    acceptance = factor_tests.get("acceptance_contract") if isinstance(factor_tests.get("acceptance_contract"), dict) else {}
    if acceptance:
        acceptance = dict(acceptance)
        acceptance["durable_evidence_recipe_ready"] = bool(recipe.get("local_recipe_ready"))
        acceptance["durable_evidence_recipe_status"] = recipe.get("status")
        acceptance["durable_evidence_recipe_is_not_provider_validation"] = True
        acceptance["durable_evidence_recipe_is_not_production_completion"] = True
        acceptance["durable_evidence_production_blocker_count"] = recipe.get("production_blocker_count")
        acceptance["provider_execution_implemented"] = False
        acceptance["provider_backed_small_pool_validation_done"] = False
        acceptance["full_market_validation_done"] = False
        acceptance["production_factor_test_validation_complete"] = False
...
def run_factor_test_provider_small_pool_execution_request_task(payload: Any = None) -> dict[str, Any]:
    now = _now_iso()
    ...
    factor_tests = dict(factor_tests)
    payload_safe = _factor_test_provider_small_pool_execution_request_payload(payload, factor_tests, now)
    receipt, rows = _factor_test_provider_small_pool_execution_request_receipt(payload_safe, now)
    payload_safe["provider_small_pool_execution_request_receipt"] = receipt
    payload_safe["provider_small_pool_execution_request_rows"] = rows
    task = create_task_record(
        "run_factor_test_provider_small_pool_execution_request",
        output_packet_key="command_center_factor_quant_hub_packet",
        payload=payload_safe,
        current_step="factor_test_provider_small_pool_execution_request_queued",
        warnings=[
            "Factor Test provider 小股票池 execution request 只生成本地请求 ticket，不调用 Tushare、DeepSeek 或 GitHub。",
            "execution request 不创建 provider task、不计算生产 IC/Rank IC/ICIR、不修改 strategy action、不执行真实交易。",
        ],
    )
    if task.get("dedupe_reused_existing"):
        return task
    update_task_status(
        ...
        status="running",
        progress=0.25,
        current_step="building_factor_test_provider_small_pool_execution_request_ticket",
    )
    ...
    try:
        ...
        hub["factor_tests"] = factor_tests
        hub["call_ledger"] = list(receipt.get("call_ledger") or []) + list(hub.get("call_ledger") if isinstance(hub.get("call_ledger"), list) else [])
        warning = "Factor Test provider 小股票池 execution request ticket 已生成：本地请求，不调用 provider，不代表生产验收完成。"
        existing_warnings = hub.get("warnings") if isinstance(hub.get("warnings"), list) else []
        hub["warnings"] = [warning] + [item for item in existing_warnings if item != warning]
        hub["external_calls_triggered"] = False
        hub["tushare_called"] = False
...
def _factor_universe_worker_batch_execution_request_row(
    criterion: str,
    status: str,
    passed: bool,
    evidence: str,
    ...
    *,
    ...
    return {
        ...
        "status": status,
        "passed": bool(passed),
        "required_for_manual_worker_task_submission": bool(required),
        "blocks_manual_worker_task_submission": bool(required and not passed),
        "evidence": evidence,
        "next_action": next_action,
        "worker_task_created": False,
        "worker_task_executed": False,
...
def _factor_universe_worker_batch_execution_request_payload(
    payload: Any,
    packet: dict[str, Any],
    now: str,
) -> dict[str, Any]:
    dry_run = _dict(packet.get("universe_worker_batch_dry_run_receipt"))
    recipe = _dict(packet.get("universe_worker_batch_execution_recipe"))
    payload_dict = payload if isinstance(payload, dict) else {}
    requested_scope_hash = str(
    ...
    return {
        ...
        "requested_scope_hash": requested_scope_hash,
        "latest_dry_run_scope_hash": str(dry_run.get("worker_batch_scope_hash") or ""),
        "latest_dry_run_scope_hash_short": str(dry_run.get("worker_batch_scope_hash_short") or ""),
        "latest_dry_run_status": str(dry_run.get("status") or ""),
        "execution_recipe_status": str(recipe.get("status") or ""),
        "execution_recipe_ready": bool(recipe.get("local_recipe_ready")),
        "scope_ticket_ready": bool(recipe.get("scope_ticket_ready")),
        "universe_mode": dry_run.get("universe_mode"),
        "symbols": [str(item) for item in dry_run.get("symbols", []) if item][:FACTOR_UNIVERSE_WORKER_BATCH_SYMBOL_LIMIT],
        "symbol_count": int(dry_run.get("symbol_count") or 0),
        "required_datasets": list(FACTOR_UNIVERSE_RESEARCH_PLAN_DATASETS),
...
def _factor_universe_worker_batch_execution_request_receipt(
    payload_safe: dict[str, Any],
    now: str,
) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    latest_scope_hash = str(payload_safe.get("latest_dry_run_scope_hash") or "")
    ...
    dry_run_visible = bool(
        payload_safe.get("latest_dry_run_status")
        and latest_scope_hash
        and str(payload_safe.get("latest_dry_run_status")) != "worker_batch_dry_run_blocked_preflight"
    )
    user_confirmed = payload_safe.get("approved_by_user") is True
    recipe_ready = bool(payload_safe.get("execution_recipe_ready") and payload_safe.get("scope_ticket_ready"))
    ...
    rows = [
        _factor_universe_worker_batch_execution_request_row(
            ...
            "passed_latest_scope_ticket_visible" if dry_run_visible else "blocked_missing_or_blocked_worker_batch_scope_ticket",
            dry_run_visible,
            f"latest_dry_run_status={payload_safe.get('latest_dry_run_status')}; scope_hash_short={payload_safe.get('latest_dry_run_scope_hash_short')}",
            "Run the worker-batch dry-run first and keep the approved scope hash visible.",
        ),
        _factor_universe_worker_batch_execution_request_row(
            "scope_hash_bound_to_latest_dry_run",
            "passed_scope_hash_match" if requested_hash_matches_latest else "blocked_scope_hash_missing_or_mismatch",
            requested_hash_matches_latest,
            f"requested_scope_hash_short={requested_scope_hash[:16]}; latest_scope_hash_short={payload_safe.get('latest_dry_run_scope_hash_short')}",
            "Bind the manual worker task request to the latest approved dry-run scope hash.",
        ),
        _factor_universe_worker_batch_execution_request_row(
            "explicit_user_confirmation",
            "passed_user_confirmed" if user_confirmed else "blocked_user_confirmation_required",
            user_confirmed,
            f"approved_by_user={user_confirmed}",
            "Require explicit user confirmation before a future worker-batch task is submitted.",
        ),
        _factor_universe_worker_batch_execution_request_row(
            "execution_recipe_ready",
            "passed_execution_recipe_ready" if recipe_ready else "blocked_execution_recipe_not_ready",
            recipe_ready,
            f"execution_recipe_status={payload_safe.get('execution_recipe_status')}; scope_ticket_ready={payload_safe.get('scope_ticket_ready')}",
            "Keep the ordered execution recipe visible before any worker-backed universe research.",
        ),
        _factor_universe_worker_batch_execution_request_row(
            "target_worker_task_route_declared",
            "passed_target_route_declared" if target_route_ok else "blocked_target_route_mismatch",
            target_route_ok,
        ...
        _factor_universe_worker_batch_execution_request_row(
            ...
            "passed_no_worker_task_created",
            True,
            "Execution request ticket records intent only and does not create or execute the worker task.",
            "Submit a separate worker-backed task only after that later step is explicitly approved.",
            required=False,
        ),
        _factor_universe_worker_batch_execution_request_row(
            "no_worker_provider_model_github_trade_action_side_effects",
            "passed_no_external_or_trade_side_effects",
            True,
            "Request ticket does not start workers, call Tushare, DeepSeek, GitHub, execute trades, or mutate strategy action.",
            "Keep Factor universe research-only through worker execution and promotion review.",
            required=False,
        ),
        _factor_universe_worker_batch_execution_request_row(
            "secret_redaction_boundary",
            "passed_no_secret_exposure",
            True,
            "Request ticket includes universe scope, stage scope, target route, and scope hash only.",
            "Keep token/key material out of frontend, logs, packets, cache, and task payloads.",
            required=False,
        ),
    ]
    blockers = [row["criterion"] for row in rows if row["blocks_manual_worker_task_submission"]]
    if not dry_run_visible:
        status = "factor_universe_worker_batch_execution_request_blocked_missing_dry_run_scope_ticket"
        allowed_next_step = "run_factor_universe_worker_batch_dry_run"
    elif not requested_hash_matches_latest:
        status = "factor_universe_worker_batch_execution_request_blocked_scope_hash_mismatch"
        allowed_next_step = "resubmit_execution_request_with_latest_worker_batch_scope_hash"
    elif not user_confirmed:
        status = "factor_universe_worker_batch_execution_request_blocked_user_confirmation_required"
        allowed_next_step = "confirm_worker_batch_execution_request"
    elif not recipe_ready:
        status = "factor_universe_worker_batch_execution_request_blocked_local_recipe"
        allowed_next_step = "repair_factor_universe_worker_batch_execution_recipe"
    else:
        status = "factor_universe_worker_batch_execution_request_ready_manual_worker_task_pending"
        allowed_next_step = "manual_submit_future_worker_batch_research_task_bound_to_scope_ticket"
    ready = status == "factor_universe_worker_batch_execution_request_ready_manual_worker_task_pending"
    receipt = {
        "schema_version": "factor_universe_worker_batch_execution_request.v1",
        "status": status,
        "scope": "local_factor_universe_worker_batch_execution_request_no_worker_or_provider_execution",
        "created_at": now,
        "ltg": "LTG-04/LTG-06/LTG-11/LTG-12",
        "local_execution_request_ready": ready,
        "ready_for_manual_worker_task_submission": ready,
        "cache_get_initializes_execution_request": False,
        "allowed_next_step": allowed_next_step,
        ...
        "execution_recipe_status": payload_safe.get("execution_recipe_status"),
        "requested_scope_hash_matches_latest": requested_hash_matches_latest,
        "worker_batch_scope_hash": latest_scope_hash if requested_hash_matches_latest else "",
        "worker_batch_scope_hash_short": str(payload_safe.get("latest_dry_run_scope_hash_short") or ""),
        "requested_scope_hash_short": requested_scope_hash[:16],
        "universe_mode": payload_safe.get("universe_mode"),
        "symbols": payload_safe.get("symbols") or [],
        "symbol_count": payload_safe.get("symbol_count") or 0,
        ...
        "blocking_criteria": blockers,
        "not_allowed_next_steps": [
            "treat_execution_request_as_worker_batch_execution",
            "create worker task from GET cache",
            "start worker from execution request",
            "call Tushare from execution request",
            "call DeepSeek from execution request",
            "call GitHub from execution request",
            "compute production rank/zscore from execution request",
            "mutate strategy action",
            "real trade execution",
            "leak token/key",
        ],
        "missing_evidence": [
            "separate worker-backed batch research task implementation",
            "worker task_id bound to scope hash",
    ...
    ledger = [
        {
            "api": "local_factor_universe_worker_batch_execution_request",
            "request_params_safe": {
                "status": status,
                "scope": "local_factor_universe_worker_batch_execution_request_no_worker_or_provider_execution",
                "target_worker_task_route": payload_safe.get("target_worker_task_route"),
...
def _missing_factor_universe_worker_batch_execution_request(now: str) -> tuple[dict[str, Any], list[dict[str, Any]], list[dict[str, Any]]]:
    row = _factor_universe_worker_batch_execution_request_row(
        "execution_request_task_missing",
        "no_execution_request_task_found",
        False,
        "No button-gated Factor universe worker-batch execution request ticket has been recorded yet.",
        "Run the worker-batch dry-run scope ticket, then generate an execution request ticket before the future worker task.",
    )
    receipt = {
        "schema_version": "factor_universe_worker_batch_execution_request.v1",
        "status": "factor_universe_worker_batch_execution_request_missing",
        "scope": "local_factor_universe_worker_batch_execution_request_no_worker_or_provider_execution",
        "created_at": now,
        "ltg": "LTG-04/LTG-06/LTG-11/LTG-12",
        "local_execution_request_ready": False,
        "ready_for_manual_worker_task_submission": False,
        "source_packet_present": False,
        "cache_get_initializes_execution_request": False,
        "worker_task_created": False,
        "worker_task_executed": False,
...
def _attach_factor_universe_worker_batch_execution_request(
    packet: dict[str, Any],
    now: str,
) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    receipt = packet.get("universe_worker_batch_execution_request_receipt")
    rows = packet.get("universe_worker_batch_execution_request_rows")
    if not isinstance(receipt, dict):
        receipt, rows, ledger = _missing_factor_universe_worker_batch_execution_request(now)
    else:
        receipt = dict(receipt)
        rows = rows if isinstance(rows, list) else receipt.get("rows") if isinstance(receipt.get("rows"), list) else []
        receipt["source_packet_present"] = True
        receipt["cache_get_initializes_execution_request"] = False
        ledger = list(receipt.get("call_ledger") or [])
    packet["universe_worker_batch_execution_request_receipt"] = receipt
    packet["universe_worker_batch_execution_request_rows"] = rows
    contract = packet.get("universe_research_contract")
    if isinstance(contract, dict):
        contract = dict(contract)
        contract["worker_batch_execution_request_status"] = receipt.get("status")
        contract["worker_batch_execution_request_ready"] = bool(receipt.get("local_execution_request_ready"))
        contract["worker_batch_execution_request_is_not_worker_execution"] = True
        contract["worker_task_created"] = False
        contract["worker_task_executed"] = False
        contract["worker_started"] = False
        contract["large_universe_pipeline_done"] = False
...
def _factor_universe_durable_evidence_recipe(
    ...
    rows = [
        ...
        _factor_universe_durable_evidence_recipe_row(
            "worker_batch_execution_request_visible",
            "universe_worker_batch_execution_request_receipt",
            "visible_execution_request_ticket" if execution_request_visible else "blocked_missing_worker_batch_execution_request",
            passed=execution_request_visible,
            production_blocker=False,
            local_surface_required=True,
            evidence="worker-batch execution request ticket is visible as a local scope-bound request or missing placeholder",
            next_action="Generate the request ticket from an approved dry-run before submitting a future worker task.",
        ),
        _factor_universe_durable_evidence_recipe_row(
            "explicit_worker_task_required",
            "universe_worker_batch_research_receipt",
...
def run_factor_universe_worker_batch_execution_request_task(payload: Any = None) -> dict[str, Any]:
    now = _now_iso()
    base_hub = dict(read_factor_quant_cache())
    payload_safe = _factor_universe_worker_batch_execution_request_payload(payload, base_hub, now)
    receipt, rows = _factor_universe_worker_batch_execution_request_receipt(payload_safe, now)
    payload_safe["universe_worker_batch_execution_request_receipt"] = receipt
    payload_safe["universe_worker_batch_execution_request_rows"] = rows
    task = create_task_record(
        "run_factor_universe_worker_batch_execution_request",
        output_packet_key="command_center_factor_quant_hub_packet",
        payload=payload_safe,
        current_step="factor_universe_worker_batch_execution_request_queued",
        warnings=[
            "Factor Universe worker-batch execution request 只生成本地请求 ticket，不创建任务、不启动 worker。",
            "execution request 不调用 Tushare、DeepSeek 或 GitHub，不计算 rank/zscore/neutralization，不修改 strategy action，不执行真实交易。",
        ],
    )
    if task.get("dedupe_reused_existing"):
        return task
    update_task_status(
        ...
        status="running",
        progress=0.25,
        current_step="building_factor_universe_worker_batch_execution_request_ticket",
    )
    receipt["task_id"] = task["task_id"]
    try:
        hub = dict(read_factor_quant_cache())
        hub["universe_worker_batch_execution_request_receipt"] = receipt
        hub["universe_worker_batch_execution_request_rows"] = rows
        universe_contract = hub.get("universe_research_contract") if isinstance(hub.get("universe_research_contract"), dict) else {}
        universe_contract = dict(universe_contract)
        universe_contract.update(
            {
                "worker_batch_execution_request_status": receipt.get("status"),
                "worker_batch_execution_request_ready": bool(receipt.get("local_execution_request_ready")),
                "worker_batch_execution_request_is_not_worker_execution": True,
                "worker_task_created": False,
                "worker_task_executed": False,
                "worker_started": False,
                "large_universe_pipeline_done": False,
        ...
        hub["universe_research_contract"] = universe_contract
        hub["call_ledger"] = list(receipt.get("call_ledger") or []) + list(hub.get("call_ledger") if isinstance(hub.get("call_ledger"), list) else [])
        warning = "Factor Universe worker-batch execution request ticket 已生成：本地请求，不创建/启动 worker，不代表全市场/大股票池生产研究完成。"
        existing_warnings = hub.get("warnings") if isinstance(hub.get("warnings"), list) else []
        hub["warnings"] = [warning] + [item for item in existing_warnings if item != warning]
        hub["external_calls_triggered"] = False
        hub["tushare_called"] = False
