...
def _search_quant_projection_small_data_writeback_summary(packet: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if provider_ready:
        call_ledger_surface_status = (
            f"POST task ledger 已写入：Tushare {provider_api_success_count}/{provider_api_call_count} 个接口"
        )
        call_ledger_surface_next = "回放股票量化推演和次日图谱；缺口只作为待补证据。"
    elif credential_missing_count:
        call_ledger_surface_status = "本地阻断 ledger 已写入：缺少服务端 Tushare 凭据"
        call_ledger_surface_next = "配置服务端凭据后重新点击确认；页面不会补调 provider。"
    elif provider_ledger_visible:
        call_ledger_surface_status = (
        ...
        call_ledger_surface_status = "等待确认按钮后的 POST task / worker 写入 ledger"
        call_ledger_surface_next = "先输入代码并点击确认；输入和 GET cache 不创建 provider task。"
    ordinary_writeback_surface_summary_rows = [
        {
            "surface": "cache",
            "写入面": "cache",
            "普通速读": "页面刷新后只读回放本地缓存。",
            "当前状态": "本地 cache 已写入" if cache_packet_written else "等待确认按钮写入本地 cache",
            "回放来源": "GET /api/candidate-radar/cache",
            "下一步": ordinary_readback_next_step,
            "边界": "GET cache 只读本地 packet；React render 不补调 provider/model。",
            "readback_source": "GET /api/candidate-radar/cache",
        ...
        },
        {
            "surface": "call_ledger",
            "写入面": "call_ledger",
            "普通速读": "只确认 Tushare ledger 是否由确认任务写入。",
            "当前状态": call_ledger_surface_status,
            "回放来源": "search_quant_provider_model_acceptance_receipt.provider_call_ledger",
            "下一步": call_ledger_surface_next,
            "边界": "call_ledger 只允许 POST task / worker 产生；GET cache 和 React render 不调 Tushare/DeepSeek。",
            "readback_source": "search_quant_provider_model_acceptance_receipt.provider_call_ledger",
            "cache_only_readback": True,
            "creates_task_from_readback": False,
            "readback_external_calls_triggered": False,
        ...
        },
        {
            "surface": "packet",
            "写入面": "packet",
            "普通速读": "回放 task id、安全步骤、结果入口和下一步。",
            "当前状态": f"packet={PACKET_KEY} 已写入" if cache_packet_written else "等待写入 candidate radar packet",
    ...
    if provider_ready:
        ...
        confirm_replay_stage_evidence = (
            f"provider_call_source={provider_call_source}; "
            f"provider_api_success={provider_api_success_count}/{provider_api_call_count}"
        )
    elif credential_missing_count:
        confirm_replay_stage_status = "P2 blocked：cache / ledger / packet 已写入本地阻断；缺少服务端 Tushare 凭据。"
        confirm_replay_stage_next = "配置服务端凭据后重新点击确认；GET cache 和 React render 仍只读。"
        confirm_replay_stage_evidence = "local_block_replayed_missing_tushare_credentials"
    elif latest_task_id:
        confirm_replay_stage_status = "P1 accepted：任务已接收；等待 TaskStatusPanel success 后刷新 cache。"
        confirm_replay_stage_next = "先看 TaskStatusPanel；任务完成后刷新本地 cache。"
        confirm_replay_stage_evidence = f"candidate_radar_cache_packet.task_id={latest_task_id}"
    elif quant_receipt:
        confirm_replay_stage_status = "P1 ready：点击确认后创建 Tushare-first POST task。"
        confirm_replay_stage_next = "点击确认并生成 3.0 量化推演。"
        confirm_replay_stage_evidence = "search_quant_projection_receipt"
    else:
        confirm_replay_stage_status = "等待有效股票代码；输入不会创建 task。"
        confirm_replay_stage_next = "先输入 6 位 A 股代码，再点击确认并生成。"
        confirm_replay_stage_evidence = "waiting_symbol_confirm"
    ordinary_confirm_replay_stage_rows = [
        {
            "stage_key": "current_confirm_replay_stage",
            "速读项": "P1/P2 当前阶段",
            "当前状态": confirm_replay_stage_status,
            "用户下一步": confirm_replay_stage_next,
            "证据": confirm_replay_stage_evidence,
            "边界": "阶段只由 GET cache 中的本地 task receipt / cache / ledger / packet 推导；不创建 task、不补调 provider/model。",
            "readback_source": "search_quant_projection_small_data_writeback_summary",
            "cache_only_readback": True,
            "creates_task_from_readback": False,
        ...
        {
            "stage_key": "task_acceptance",
            "速读项": "任务接收",
            "当前状态": f"task_id={latest_task_id}" if latest_task_id else "等待确认按钮返回 task id",
            "用户下一步": "看 TaskStatusPanel" if latest_task_id else "输入有效代码并点击确认",
            "证据": "candidate_radar_cache_packet.task_id" if latest_task_id else "button_not_clicked",
            "边界": "task id 只来自按钮门控 POST 或 cache packet；GET cache 不创建 task。",
            "readback_source": "candidate_radar_cache_packet",
            "cache_only_readback": True,
            "creates_task_from_readback": False,
            "uses_model_output": False,
        ...
        {
            "stage_key": "cache_ledger_packet_replay",
            "速读项": "cache / ledger / packet",
            "当前状态": summary_label,
            "用户下一步": ordinary_readback_next_step,
            "证据": f"writeback_surfaces={'/'.join(writeback_surfaces) if writeback_surfaces else 'waiting_confirm'}",
    ...
    if provider_ready:
        one_screen_primary_status = f"Tushare-first 已回放：{provider_api_success_count}/{provider_api_call_count} 个接口"
        one_screen_primary_next = "直接查看量化推演，再打开次日图谱复核本地结果。"
        one_screen_result_status = "可解释结果可读：来源=Tushare-first ledger；缺口=Factor/Next/ECharts 本地刷新。"
    elif credential_missing_count:
        one_screen_primary_status = "Tushare-first 本地阻断：缺少服务端 Tushare 凭据，provider 未调用。"
        one_screen_primary_next = "配置服务端凭据后重新点击确认按钮；页面刷新不会自动补调 provider。"
        one_screen_result_status = "可解释结果只显示阻断原因；还没有真实 provider ledger。"
    elif provider_ledger_visible:
        one_screen_primary_status = f"Tushare-first 部分回放：{provider_api_success_count}/{provider_api_call_count} 个接口"
        one_screen_primary_next = "补齐 Tushare light ledger 后再查看量化推演和次日图谱。"
        one_screen_result_status = "可解释结果部分可读；缺口保持为待补证据。"
    elif latest_task_id:
        one_screen_primary_status = f"确认任务已接收：{latest_task_id}"
        one_screen_primary_next = "先看 TaskStatusPanel；任务完成后刷新本地 cache 回放。"
        one_screen_result_status = "结果入口等待任务完成并写回 cache / ledger / packet。"
    else:
        one_screen_primary_status = "等待输入股票代码并点击确认。"
        one_screen_primary_next = "先输入 6 位 A 股代码，再点击确认并生成 3.0 量化推演。"
        one_screen_result_status = "结果入口待确认；输入和搜索不会创建 task。"
    ordinary_one_screen_action_rows = [
        {
            "action_key": "confirm",
            "行动": "1. 确认",
            "当前状态": one_screen_primary_status,
            "用户下一步": one_screen_primary_next,
            "入口": "确认并生成 3.0 量化推演",
            "证据": (
                f"task_id={latest_task_id}"
                if latest_task_id
                else f"symbol={chain_symbol}" if chain_symbol else "waiting_symbol_confirm"
            ),
        ...
        },
        {
            "action_key": "task",
            "行动": "2. 任务",
            "当前状态": "TaskStatusPanel 可本地轮询" if latest_task_id else "等待确认按钮返回 task id",
            "用户下一步": "有 task id 时看本地任务状态；没有 task id 时先点击确认按钮。",
            "入口": "TaskStatusPanel",
            "证据": "local_task_status" if latest_task_id else "button_not_clicked",
            "边界": "任务状态只读本地 FastAPI；不从轮询行调用 Tushare、DeepSeek、GitHub 或交易路径。",
            "readback_source": "local_task_status",
            "cache_only_readback": True,
            "creates_task_from_readback": False,
            "readback_external_calls_triggered": False,
        ...
        {
            "action_key": "writeback",
            "行动": "3. 写回",
            "当前状态": ordinary_readback_stage_label,
            "用户下一步": ordinary_readback_next_step,
            "入口": "cache / call_ledger / packet",
            "证据": summary_label,
            "边界": "写回回放只读 cache / ledger / packet；不会补调 provider/model、不泄露 key。",
            "readback_source": "GET /api/candidate-radar/cache",
            "cache_only_readback": True,
            "creates_task_from_readback": False,
            "readback_external_calls_triggered": False,
    ...
    return {
        ...
        "ordinary_writeback_surface_summary_rows": ordinary_writeback_surface_summary_rows,
        "ordinary_writeback_surface_summary_row_count": len(ordinary_writeback_surface_summary_rows),
        ...
        "ordinary_one_screen_action_rows": ordinary_one_screen_action_rows,
        "ordinary_one_screen_action_row_count": len(ordinary_one_screen_action_rows),
        "ordinary_one_screen_action_rows_are_cache_only": True,
        "ordinary_one_screen_action_rows_create_task": False,
        "ordinary_one_screen_action_rows_call_provider_from_readback": False,
        "ordinary_one_screen_action_rows_use_model_output": False,
        "ordinary_one_screen_action_rows_are_not_trade_signals": True,
        ...
        "ordinary_confirm_replay_stage_rows_create_task": False,
        "ordinary_confirm_replay_stage_rows_use_model_output": False,
        "ordinary_confirm_replay_stage_rows_are_not_trade_signals": True,
...
def _attach_search_quant_projection_small_data_writeback_summary(packet: Mapping[str, Any]) -> dict[str, Any]:
    ...
    counts["search_quant_projection_one_screen_action_row_count"] = summary.get(
        "ordinary_one_screen_action_row_count", 0
    )
    ...
    policy["search_quant_projection_one_screen_action_rows_are_cache_only"] = True
    policy["search_quant_projection_one_screen_action_rows_create_task"] = False
    policy["search_quant_projection_one_screen_action_rows_call_provider_from_readback"] = False
    policy["search_quant_projection_one_screen_action_rows_use_model_output"] = False
    policy["search_quant_projection_one_screen_action_rows_are_not_trade_signals"] = True
...
def _search_quant_projection_interpretation_summary(packet: Mapping[str, Any]) -> dict[str, Any]:
    ...
    ordinary_deepseek_governed_executor_checklist_rows = [
        {
            "check_key": "model_ledger_gate",
            "检查项": "model_ledger 门控",
            "当前状态": (
                "model_ledger 已可回放；仍只展示安全摘要。"
                if deepseek_model_ledger_ready
                else "待 P5 governed executor 写入 model_ledger。"
            ),
            "用户下一步": "没有 model_ledger 前，普通页只使用 Tushare-first 与本地图谱。",
            "证据": (
                f"deepseek_requested={deepseek_requested}; "
                f"deepseek_skipped={deepseek_skipped}; "
                f"model_ledger_ready={deepseek_model_ledger_ready}"
            ),
            "边界": "DeepSeek 不作为数据源；GET cache 和 React render 不调用模型。",
            "readback_source": "search_quant_projection_interpretation_summary",
            "cache_only_readback": True,
            "creates_task_from_readback": False,
        ...
        {
            "check_key": "sanitizer_redaction",
            "检查项": "sanitizer / redaction",
            "当前状态": "真实模型输出进入普通页前必须先完成字段白名单、脱敏和安全摘要。",
            "用户下一步": "P5 单独补 sanitizer / redaction 后再考虑展示解释缓存。",
            "证据": "allowed_fields=source/gap/next_step/safety_summary; raw_prompt_output_forbidden",
            "边界": "token/key、raw prompt、raw output、未脱敏 provider error 不进前端、日志、packet 或 cache。",
            "readback_source": "local_model_governance_policy",
            "cache_only_readback": True,
            "creates_task_from_readback": False,
            "calls_model_from_readback": False,
        ...
        {
            "check_key": "safe_fallback",
            "检查项": "安全回退",
            "当前状态": "parse failed、超时或缺 model_ledger 时只显示 pending/skipped，不影响 Tushare 和基础图谱。",
            "用户下一步": "继续使用 cache / ledger / packet；把 DeepSeek 当后续独立补证。",
            "证据": f"tushare_ledger_ready={small_data_ready}; factor_next_ready={factor_next_ready}",
            "边界": "DeepSeek 失败不能阻塞 P1/P2/P3，也不能触发重试外联或写交易动作。",
            "readback_source": "cache / call_ledger / packet",
            "cache_only_readback": True,
            "creates_task_from_readback": False,
            "calls_model_from_readback": False,
        ...
        {
            "check_key": "no_override",
            "检查项": "不覆盖数值或动作",
            "当前状态": "后续 DeepSeek 只能解释已有证据，不能覆盖价格、持仓、factor、operation_zones 或 strategy action。",
            "用户下一步": "只把模型解释当研究说明；交易路径继续隔离。",
            "证据": "no_override_price_position_factor_operation_zones_strategy_action",
            "边界": "DeepSeek 文本不是买入/卖出/加减仓指令。",
            "readback_source": "local_model_governance_policy",
            "cache_only_readback": True,
            "creates_task_from_readback": False,
            "calls_model_from_readback": False,
    ...
    return {
        ...
        "ordinary_deepseek_governed_executor_checklist_rows": ordinary_deepseek_governed_executor_checklist_rows,
        "ordinary_deepseek_governed_executor_checklist_row_count": len(ordinary_deepseek_governed_executor_checklist_rows),
        "ordinary_deepseek_governed_executor_checklist_rows_are_cache_only": True,
        "ordinary_deepseek_governed_executor_checklist_rows_create_task": False,
...
def _attach_search_quant_projection_interpretation_summary(packet: Mapping[str, Any]) -> dict[str, Any]:
    ...
    policy["search_quant_projection_deepseek_governed_executor_checklist_rows_are_cache_only"] = True
    policy["search_quant_projection_deepseek_governed_executor_checklist_rows_create_task"] = False
