...
def _one_click_startup_summary(
    ...
    ordinary_recovery_steps = [
        ...
        },
        {
            ...
            "title": "按启动器诊断定位失败段",
            "when": "启动器没有自动打开页面",
            "action": "先运行 scripts/check_command_center_3.command 做 check-only 安全自检，再看 FastAPI、bootstrap status、desktop preflight cache、React/Vite 哪一段没有 ready。",
            "checks": "check-only 不启动 FastAPI/Vite、不探测 URL、不打开浏览器；再对应检查 8710/5173 端口占用和 .stock_ming_3/logs 下的 fastapi/vite 日志。",
            "external_calls_triggered": False,
            "tushare_called": False,
            "deepseek_called": False,
            "github_called": False,
    ...
    return {
        ...
        "blocked_next_action": "若未打开页面，先运行 scripts/check_command_center_3.command 做 check-only 安全自检，再看启动器的可操作诊断：FastAPI、bootstrap status、desktop preflight cache、React/Vite 哪段失败；再检查 8710/5173 是否被占用，或进入桌面壳预检查看 P0 四段联通；P0 未 ready 时不要进入 P1 确认按钮；日志在 .stock_ming_3/logs/command_center_3_fastapi.log 与 command_center_3_vite.log。",
        "safe_check_command": "scripts/check_command_center_3.command",
        "safe_check_boundary": "check-only 安全自检只解析本地启动配置；不启动 FastAPI/Vite、不探测 URL、不打开浏览器、不创建 task、不调用 provider/model。",
        "diagnostic_surfaces": [
        ...
        "success_handoff_label": "联通成功后回到今日作战台首页确认股票代码；需要详情再打开下一票雷达。只有确认按钮创建 Tushare-first POST task，DeepSeek 保持 governed/skipped。",
        "success_handoff_href": HOME_CONFIRM_HREF,
        "success_handoff_boundary": "启动器和预检页主入口只回首页确认；页面切换和输入不外联，确认按钮才进入 P1 task。",
...
def _p0_ordinary_connection_rows(one_click_startup_summary: dict[str, Any]) -> list[dict[str, Any]]:
    def row(
        ...
        launcher_output_anchor: str,
        page_readback_location: str,
        log_or_port: str,
        ...
        return {
            ...
            "启动器输出锚点": launcher_output_anchor,
            "页面回读位置": page_readback_location,
            "日志/端口": log_or_port,
            "可执行检查": executable_check,
            "边界": boundary,
            "ordinary_user_visible": True,
    ...
    return [
        row(
            ...
            "系统健康页 / 今日作战台：本地联通、FastAPI health",
            ".stock_ming_3/logs/command_center_3_fastapi.log / 8710",
            "确认 health JSON service=stock-MING Command Center 3.0 且 external_calls_on_startup=false",
        ...
        row(
            ...
            "确认 packet_key=command_center_3_bootstrap_runtime_mode_packet",
        ...
        row(
            ...
            "桌面壳预检 / 系统健康 / 今日作战台：P0 一键启动 packet",
            ".stock_ming_3/logs/command_center_3_fastapi.log / /api/desktop/preflight-cache",
            "确认 packet_key=command_center_3_desktop_shell_preflight_cache",
        ...
        row(
            ...
            "浏览器普通首页 #home：今日作战台摘要",
            ".stock_ming_3/logs/command_center_3_vite.log / 5173",
            "确认页面标题/HTML 包含 stock-MING Command Center 3.0 和 /src/main.tsx",
...
def _p0_to_p1_ordinary_handoff_rows(one_click_startup_summary: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    def boundary_fields() -> dict[str, Any]:
        return {
            "P0交接证据": f"四段 ready 后主入口切到 {HOME_CONFIRM_HREF} 首页确认股票代码；需要详情再打开 {CANDIDATE_CONFIRM_HREF}；输入股票代码保持静默；确认按钮才创建 Tushare-first POST task。",
    ...
    return [
        {
            ...
            "下一步": "回到今日作战台首页确认股票代码；需要详情再打开下一票雷达。" if connection_ready else "回到启动器诊断或桌面壳预检。",
...
def _p0_ordinary_quick_action_rows(
    ...
    return [
        row(
            ...
            "去今日作战台首页确认股票代码" if connection_ready else "回一键启动预检看 FastAPI / bootstrap / React 哪段失败",
            "今日作战台摘要",
        ...
        row(
            "2. 首页确认股票代码",
            "只读导航",
            "在首页确认卡输入股票代码，先做本地格式校验；需要详情再进下一票雷达",
            "今日作战台首页确认卡",
            "页面切换和输入不会创建 task，也不会调用 Tushare、DeepSeek 或 GitHub；下一票雷达只作为详情入口。",
            1,
        ),
        row(
            "3. 确认并生成",
...
def _p0_current_next_action_rows(
    ...
    connection_ready = one_click_startup_summary.get("frontend_backend_connection_ready") is True
    primary_entry = HOME_CONFIRM_HREF if connection_ready else "#desktop"
    primary_next = "回到今日作战台首页确认股票代码；输入保持静默，确认按钮才触发 Tushare-first；需要详情再进下一票雷达。" if connection_ready else "留在桌面壳预检，按 FastAPI / bootstrap status / desktop preflight cache / React/Vite 四段恢复。"
...
def _p0_startup_30s_quick_read_rows(
    one_click_startup_summary: dict[str, Any],
    ...
    next_action = str(p0_current_next_action_rows[0].get("用户下一步") or "") if p0_current_next_action_rows else ""
    failed_segments = [
        str(row.get("失败段") or "")
    ...
    return [
        row(
            "1. 页面是否应该打开",
            "ready：启动器可打开普通首页" if connection_ready else "check：启动器不应自动打开页面",
            "如果页面没有打开，先留在启动器诊断；如果已打开，继续看四段联通。",
            str(one_click_startup_summary.get("success_condition") or ""),
            "启动器只在 FastAPI、bootstrap status、desktop preflight cache、React/Vite 全部 ready 后打开页面。",
            str(one_click_startup_summary.get("what_user_should_click_next") or "scripts/start_command_center_3.command"),
        ),
        row(
            "2. 四段联通",
            f"{ready_count}/4 ready",
            "少于 4/4 时按失败段处理；4/4 时回首页确认股票代码。",
        ...
        row(
            ...
            "失败定位不读取 token/key，不调用 Tushare/DeepSeek/GitHub，不执行真实交易。",
            ".stock_ming_3/logs",
        ),
        row(
            "5. 现在下一步",
            ...
            HOME_CONFIRM_HREF if connection_ready else "#desktop",
        ),
        row(
            "6. 本轮边界",
            "只读预检回放",
            ...
            "不启用 live_light，不调用 provider/model，不写 cache，不交易，不改 strategy action。",
            "ordinary_p0_summary",
        ),
    ]
...
def _p0_ordinary_one_screen_rows(
    ...
    fallback_entry = HOME_CONFIRM_HREF if connection_ready else "#desktop"
    current_entry = (
    ...
    return [
        ...
        row(
            ...
            "4/4 ready 后回首页确认股票代码；少于 4/4 时按失败段看日志和端口。",
            "FastAPI health + bootstrap status + desktop preflight cache + React/Vite HTML",
            "#desktop",
        ...
        row(
            "3. 进入 P1 确认",
            "ready：可以回首页确认股票代码" if connection_ready else "blocked：P0 未 ready，先留在预检页",
            current_next or ("回首页输入股票代码；确认按钮才触发 Tushare-first。" if connection_ready else "留在桌面壳预检，按四段恢复。"),
            "p0_current_next_action_rows",
            current_entry,
...
def read_desktop_shell_preflight_cache() -> dict[str, Any]:
    ...
    p0_startup_30s_quick_read_rows = _p0_startup_30s_quick_read_rows(
        one_click_startup_summary,
    ...
    packet = {
        ...
        "counts": {
            ...
            "p0_startup_30s_quick_read_row_count": len(p0_startup_30s_quick_read_rows),
        ...
        "policy": {
            ...
            "p0_startup_30s_quick_read_rows_are_cache_only": True,
            "p0_startup_30s_quick_read_rows_do_not_start_services": True,
            "p0_startup_30s_quick_read_rows_do_not_create_task": True,
            "p0_startup_30s_quick_read_rows_do_not_call_provider_model": True,
        ...
        + [
            ...
            {
                "api": "local_p0_startup_30s_quick_read_rows",
