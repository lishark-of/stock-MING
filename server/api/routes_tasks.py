...
@router.post("/tushare-provider-target-sample-execution-recipe-seed")
def tushare_provider_target_sample_execution_recipe_seed(payload: dict[str, Any] | None = None) -> dict:
    task = tushare_task_service.run_tushare_provider_target_sample_execution_recipe_seed(payload)
    return task_envelope(task)
...
@router.post("/tushare-provider-target-sample-failure-window-review")
def tushare_provider_target_sample_failure_window_review(payload: dict[str, Any] | None = None) -> dict:
    task = tushare_task_service.run_tushare_provider_target_sample_failure_window_review(payload)
    return task_envelope(task)
...
@router.post("/tushare-alternative-hard-risk_evidence-scope-ticket")
def tushare_alternative_hard_risk_evidence_scope_ticket(payload: dict[str, Any] | None = None) -> dict:
    task = tushare_task_service.run_tushare_alternative_hard_risk_evidence_scope_ticket(payload)
    return task_envelope(task)
