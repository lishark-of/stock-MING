...
def render_strategy_execution_command_card(
    ...
    if a_share_fact_recovery_condition_guidance:
        group_highlight_html = "".join(
            f"<span>{escape(str(item.get('label') or 'A股事实'))} {escape(str(item.get('count') or 0))}：{escape(str(item.get('value') or '待验证'))}</span>"
            for item in (a_share_fact_recovery_condition_guidance.get("group_highlights") or [])[:3]
            if isinstance(item, dict)
        )
        condition_rows = "".join(
            f"<div class='cc-strategy-guidance-row'><span>{escape(str(item.get('label') or '门槛'))}</span><b>{escape(str(item.get('text') or '待验证。'))}</b></div>"
            for item in (a_share_fact_recovery_condition_guidance.get("condition_items") or [])[:3]
            if isinstance(item, dict)
        )
        a_share_fact_recovery_condition_html = f"""
          <div class="cc-strategy-guidance">
            <div class="cc-strategy-guidance-title">A股事实条件门槛 · {escape(str(a_share_fact_recovery_condition_guidance.get("summary") or "事实回流待验证"))}</div>
            <div class="cc-strategy-focus">{group_highlight_html}</div>
            {condition_rows}
          </div>
        """
...
def render_home_action_snapshot(snapshot: dict | None = None):
    ...
    if candidates:
        ...
        for item in candidates[:3]:
            ...
            guardrail_dependency = item.get("execution_guardrail_dependency_summary") if isinstance(item.get("execution_guardrail_dependency_summary"), dict) else {}
            ...
            guardrail_dependency_text = "；".join(
                f"{_home_text(row.get('label'), '执行护栏')}:{_home_text(row.get('status_label'), '待验证')}"
                for row in (item.get("execution_guardrail_dependencies") or [])[:3]
                if isinstance(row, dict)
            ) or "公告/硬风险与交易纪律待验证"
            candidate_html += f"""
            <div class="cc-home-candidate">
              ...
              <div class="cc-home-item-meta">执行护栏：<span class="cc-home-chip {escape(_home_text(guardrail_dependency.get("tone"), "missing"))}">{escape(_home_text(guardrail_dependency.get("label"), "护栏待验证"))}</span> {escape(_home_text(guardrail_dependency.get("summary"), "已回流 0｜缓存 0｜仍阻断 0｜待验证 0"))} ｜ {escape(guardrail_dependency_text)}</div>
    ...
    if etfs:
        ...
        for item in etfs[:3]:
            ...
            etf_guardrail_dependency = item.get("execution_guardrail_dependency_summary") if isinstance(item.get("execution_guardrail_dependency_summary"), dict) else {}
            ...
            etf_guardrail_dependency_text = "；".join(
                f"{_home_text(row.get('label'), '执行护栏')}:{_home_text(row.get('status_label'), '待验证')}"
                for row in (item.get("execution_guardrail_dependencies") or [])[:3]
                if isinstance(row, dict)
            ) or "公告/硬风险与交易纪律待验证"
            ...
            <div class="cc-home-etf">
              ...
              <div class="cc-home-item-meta">执行护栏：<span class="cc-home-chip {escape(_home_text(etf_guardrail_dependency.get("tone"), "missing"))}">{escape(_home_text(etf_guardrail_dependency.get("label"), "护栏待验证"))}</span> {escape(_home_text(etf_guardrail_dependency.get("summary"), "已回流 0｜缓存 0｜仍阻断 0｜待验证 0"))} ｜ {escape(etf_guardrail_dependency_text)}</div>
...
def render_command_center_projection_chart(projection_packet: dict | None = None):
    ...
    fact_recovery_detail_items = [item for item in (payload.get("path_fact_recovery_detail_items") or []) if isinstance(item, dict)]
    ...
    fact_detail_html = ""
    for item in fact_recovery_detail_items[:3]:
        item_tone = str(item.get("tone") or "missing")
        item_tone_class = {
            "ready": "green",
            "success": "green",
        ...
        label = str(item.get("label") or "A股事实")
        value = str(item.get("value") or item.get("path_impact") or "待验证")
        fact_detail_html += (
            f"<span class='projection-fact-pill {escape(item_tone_class)}'>"
    ...
    if data_capability_summary or evidence_group_summary or fact_recovery_summary or fact_recovery_items or legacy_chain_summary:
        ...
        <div class="projection-fact-strip">
          ...
          <div class="projection-fact-pills">{fact_detail_html}</div>
